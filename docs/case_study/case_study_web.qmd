...
# Use project_root from setup chunk
qmd_dir <- file.path(project_root, "docs", "case_study")
images_dir <- file.path(qmd_dir, "images")
if (!dir.exists(images_dir)) dir.create(images_dir, recursive = TRUE)

# Try multiple possible source paths
src_paths <- c(
  file.path(project_root, "outputs", "LBA", "lba_trace_plot.png"),
  file.path(getwd(), "outputs", "LBA", "lba_trace_plot.png"),
  "../outputs/LBA/lba_trace_plot.png",
  "../../outputs/LBA/lba_trace_plot.png"
)
...
if (!is.null(src_file)) {
  ...
  cat("⚠ Trace plot not found. Tried paths:\n")
  for (path in src_paths) {
    cat("  -", path, "\n")
  }
}
