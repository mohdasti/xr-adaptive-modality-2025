...
<div class="tldr-card" role="note" aria-label="TL;DR">
  ...
  <div class="tldr-kpis">
    ...
    <div class="kpi">
      ...
      <div class="val">TP`r tip_tp` · Error · MT · TLX`r tip_tlx`</div>
    </div>
  </div>
...
## Experimental Procedure & Participant Journey {#screenshots}
...
-   **Factorial Conditions**: 2×2×2 within-subjects design (Modality:
    Hand/Gaze × UI Mode: Static/Adaptive × Pressure: OFF/ON)
...
-   **Telemetry Schema:** Comprehensive event logging (trial start/end,
    policy changes, display violations, movement samples)
-   **Adaptation Policy Engine:** Threshold-based triggers with
    hysteresis (`app/src/lib/policy.ts`)
-   **Automated Report:** Quarto-based reproducible analysis pipeline
    (`Report.qmd` → `Report.html`)

**Key Features:**

-   Remote data collection (N=`r if(!is.na(n_total)) n_total else "—"`
    participants total)
-   ISO
    9241-9`r tip_i("International standard for evaluating pointing devices. Defines Fitts' Law task structure, effective width calculation (We = 4.133 × SD), and Throughput metric.")`
    compliant Fitts'
    Law`r tip_i("Standard HCI benchmark for target acquisition. Difficulty (ID) increases with distance and decreases with target width. Widely used to compare input methods.")`
    task
-   Real-time adaptation engine with hysteresis
-   Comprehensive telemetry (movement kinematics, display metadata,
    policy state)
-   Automated quality control (invalid trial detection, exclusion
    criteria)
...
1.  **Hand input outperformed gaze:** Higher
    throughput`r tip_i("Throughput (TP): speed–accuracy efficiency metric. TP = IDe / MT, where IDe is effective difficulty and MT is movement time. Higher TP = better efficiency.")`
    (5.15 vs 4.70 bits/s), lower error rate (1.7% vs 18.6%), lower
    workload (40.4 vs 47.0
    NASA-TLX`r tip_i("NASA Task Load Index: 6 subscales (Mental, Physical, Temporal, Performance, Effort, Frustration). We report unweighted average (0–100). Higher = more workload. Useful when performance differences are small but users feel the task is harder.")`)
2.  **Gaze errors are predominantly slips (99.2%),** not
    timeouts—validating the Midas Touch problem in XR gaze interaction
3.  **Adaptive UI (declutter) showed modest benefits** in gaze: slightly
    lower error rate (18.2% vs 19.1%)
...
We used a psychophysics-inspired gaze proxy (jitter + saccadic suppression + confirm). See [Appendix figure](#psychophysics-fig).
...
To compare hand vs gaze fairly, we used an ISO
9241-9`r tip_i("International standard for evaluating pointing devices. Defines Fitts' Law task structure, effective width calculation (We = 4.133 × SD), and Throughput metric.")`
style Fitts'
Law`r tip_i("Standard HCI benchmark for target acquisition. Difficulty (ID) increases with distance and decreases with target width. Widely used to compare input methods.")`
pointing task — the standard HCI benchmark for target acquisition
(selection). It's widely used because it isolates the core interaction
loop ("move → acquire → confirm") while letting us systematically
control difficulty via target size and distance. That makes it ideal for
studying XR selection failure modes (e.g., gaze slips / Midas Touch) and
testing UI assistance policies under time pressure. Selection is one of
the highest-frequency actions in XR UI; benchmarking target acquisition
gives direct insight into practical UI reliability.

Rather than looking at speed and errors separately, we report
**Throughput
(TP)**`r tip_i("Speed–accuracy efficiency metric: TP = IDe / MT. Higher TP means users acquire targets faster without sacrificing precision. Prevents misleading 'accuracy by slowing down' interpretations.")`
as the primary performance metric. TP summarizes how efficiently users
can acquire targets by combining **movement time** and **effective
accuracy** into a single value (higher is better). In practice, TP helps
avoid misleading conclusions like "faster, but much sloppier" or "more
accurate, but only because users slowed down."

```{r task-layout-fig, echo=FALSE, out.width="100%"}
#| fig-cap: "Task UI anatomy. Participants completed an ISO 9241-9 Fitts-style pointing task on a central canvas, with Task Control (progress + block) on the left and a System HUD (modality + block errors) on the right. Hand used visible cursor + click; Gaze used a jittering gaze dot + SPACE confirmation."
...
::: callout-tip
**TP is a speed–accuracy efficiency score:** It rises when users get
faster *without* sacrificing precision. Adaptations can "improve
accuracy" by slowing people down; TP makes that tradeoff explicit.
:::
...
**Conditions:** - Modality: Hand (mouse) vs. Gaze (simulated) - UI Mode:
Static vs. Adaptive
(declutter`r tip_i("Declutter: adaptive UI policy that removes non-essential visual elements (e.g., background decorations, secondary buttons) to reduce visual noise and improve target visibility.")`
for gaze; width
inflation`r tip_i("Width inflation: adaptive UI policy that increases target width (width_scale_factor > 1.0) to make targets easier to acquire. Not activated in this dataset.")`
for hand, but not activated) - Pressure: Low (0) vs. High (1) cognitive
load
...
<summary><strong>Technical Details & Statistical Analysis</strong>
...
#### Gaze proxy / psychophysics {#psychophysics-fig}
...
-   **Experiment Engine:** Block/condition assignment using Williams
    counterbalancing sequences
-   **Telemetry Schema:** Per-trial logging includes: trial metadata
    (modality, ui_mode, pressure), performance metrics (RT, correctness,
    error type), Fitts' parameters (A, W, ID), endpoint coordinates,
    movement samples (trajectory), display state (zoom, fullscreen, tab
    visibility), policy state (adaptation triggers, actions)
-   **QC Signals:** Zoom level, fullscreen mode, tab hidden duration,
    focus/blur events, RT bounds, input device type
-   **Deployment:** Vercel with automated CI/CD

**Architecture:**

```{r architecture-fig, echo=FALSE, out.width="100%"}
...
```{r telemetry-schema-table, echo=FALSE}
# Create telemetry schema table
telemetry_schema <- tibble(
  Category = c(
    "Trial Metadata", "Trial Metadata", "Trial Metadata", "Trial Metadata",
    "Performance Metrics", "Performance Metrics", "Performance Metrics", "Performance Metrics",
    "Fitts' Parameters", "Fitts' Parameters", "Fitts' Parameters", "Fitts' Parameters",
    "Endpoint Data", "Endpoint Data", "Endpoint Data", "Endpoint Data",
    "Movement Samples", "Movement Samples", "Movement Samples", "Movement Samples", "Movement Samples",
    "Display State", "Display State", "Display State",
    "Policy State", "Policy State"
  ),
  Field = c(
    "trial_id", "session_id", "participant_id", "taskType",
    "rt_ms", "correct", "movement_time_ms", "submovement_count",
    "A", "W", "ID", "target_width_px",
    "endpoint_x", "endpoint_y", "endpoint_error_px", "target_center_x",
    "path_length_px", "peak_velocity_px_s", "curvature_index", "sd_along_px", "sd_ortho_px",
    "screen_width", "screen_height", "device_pixel_ratio",
    "adaptation_triggered", "width_scale_factor"
  ),
  Type = c(
    "string", "string", "string", "string",
    "number (ms)", "boolean", "number (ms)", "integer",
    "number (px)", "number (px)", "number (bits)", "number (px)",
    "number (px)", "number (px)", "number (px)", "number (px)",
    "number (px)", "number (px/s)", "number", "number (px)", "number (px)",
  ...
  Description = c(
    ...
    "Task type ('point' or 'drag')",
    "Reaction time (stimulus onset to selection)",
    "Trial success (endpoint within target)",
    "Movement time (first to last sample)",
    "Number of submovements (speed minima)",
    "Amplitude (distance to target center)",
    "Target width",
    "Index of Difficulty (log₂(2A/W))",
    "Target width in pixels",
    "Selection endpoint X coordinate",
    "Selection endpoint Y coordinate",
    "Euclidean distance from endpoint to target center",
    "Target center X coordinate",
    "Total path length traveled",
    "Peak velocity during movement",
    "Path curvature (path length / straight-line distance)",
    "Standard deviation along movement axis",
    "Standard deviation orthogonal to movement axis",
    "Screen width reported by browser",
    "Screen height reported by browser",
    "Device pixel ratio",
    "Whether adaptive policy triggered this trial",
    "Width scale factor applied (1.0 = no change, >1.0 = wider targets)"
  )
)

if (has_gt && nrow(telemetry_schema) > 0) {
  telemetry_schema %>%
    ...
    tab_header(
      title = "Telemetry Schema: Key Fields",
      subtitle = "Per-trial logging includes trial metadata, performance metrics, Fitts' parameters, endpoint data, movement samples, display state, and policy state"
    ) %>%
    opt_table_font(font = "system-ui, -apple-system") %>%
    opt_table_lines(extent = "none") %>%
    tab_options(
  ...
  cat("**Telemetry Schema: Key Fields**\n\n")
  cat("| Category | Field | Type | Description |\n")
  cat("|----------|-------|------|-------------|\n")
  for (i in 1:nrow(telemetry_schema)) {
    cat(sprintf("| %s | `%s` | %s | %s |\n",
                telemetry_schema$Category[i],
                telemetry_schema$Field[i],
                telemetry_schema$Type[i],
                telemetry_schema$Description[i]))
  }
}
```
...
components, revealing how modality and pressure affect decision-making
processes.

**Inputs needed:**

Trial-level RT + correctness data, separated by modality × pressure
conditions.

**Planned output:**

Forest plot of LBA parameters (drift, threshold, NDT) by modality ×
pressure, showing how gaze vs hand differ in information accumulation
