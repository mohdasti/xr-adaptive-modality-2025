...
format:
  html:
    ...
    fig-width: 10
    fig-height: 6
    css: docs/assets/css/case-study.css
---
...
<div class="tldr-card" role="note" aria-label="TL;DR">
  ...
  <div class="tldr-text">
    I built a remote XR interaction testbed and evaluated whether a <b>context-aware adaptive system</b> can reduce gaze failure modes (e.g., <em>Midas Touch</em>) and workload under time pressure.
  </div>
  ...
  <div class="tldr-kpis">
    ...
    <div class="kpi">
      ...
      <div class="val">TP`r tip_i("Throughput: speed–accuracy efficiency metric for target acquisition. Higher TP means users acquire targets faster without sacrificing precision.")` · Error · MT · TLX`r tip_i("NASA Task Load Index: standard workload survey (0–100). Higher = more workload. Captures perceived effort and frustration.")`</div>
    </div>
  </div>

  <div class="tldr-actions">
    <!-- Demo placeholder -->
    <a class="btn primary disabled" aria-disabled="true"><i class="bi bi-play-circle"></i> Live demo (soon)</a>
    <!-- Screenshot anchor -->
    <a class="btn" href="#screenshots"><i class="bi bi-images"></i> Screenshots</a>
    <!-- Report placeholder: set href once you publish -->
    <a class="btn" href="#appendix"><i class="bi bi-file-earmark-text"></i> Appendix</a>
  </div>
    ...
    <div class="repo-bar">
      <div class="repo-badges">
        ...
        <img alt="DOI" loading="lazy"
             src="https://zenodo.org/badge/DOI/10.5281/zenodo.18204915.svg">
...
::: {.callout-note}
**Coming soon.** I'll add UI screenshots + short annotated clips from the experiment flow (hand vs gaze, pressure on/off, declutter trigger).
:::
...
- **Web Application:** React 18 + TypeScript, deployed on Vercel
- **Telemetry Schema:** Comprehensive event logging (trial start/end, policy changes, display violations, movement samples)
- **Adaptation Policy Engine:** Threshold-based triggers with hysteresis (`app/src/lib/policy.ts`)
- **Automated Report:** Quarto-based reproducible analysis pipeline (`Report.qmd` → `Report.html`)

**Key Features:**

- Remote data collection (N=`r if(!is.na(n_total)) n_total else "—"` participants total)
- ISO 9241-9`r tip_i("International standard for evaluating pointing devices. Defines Fitts' Law task structure, effective width calculation (We = 4.133 × SD), and Throughput metric.")` compliant Fitts' Law`r tip_i("Standard HCI benchmark for target acquisition. Difficulty (ID) increases with distance and decreases with target width. Widely used to compare input methods.")` task
...
## Key Results (60 Seconds) {#key-results-60-seconds}
...
We measured subjective workload using **NASA-TLX** (0–100). It's widely used in HCI because it captures perceived effort and frustration that can increase even when average performance looks similar.

**Three Takeaways:**

1. **Hand input outperformed gaze:** Higher throughput`r tip_i("Throughput (TP): speed–accuracy efficiency metric. TP = IDe / MT, where IDe is effective difficulty and MT is movement time. Higher TP = better efficiency.")` (5.15 vs 4.70 bits/s), lower error rate (1.7% vs 18.6%), lower workload (40.4 vs 47.0 NASA-TLX`r tip_i("NASA Task Load Index: 6 subscales (Mental, Physical, Temporal, Performance, Effort, Frustration). We report unweighted average (0–100). Higher = more workload. Useful when performance differences are small but users feel the task is harder.")`)
2. **Gaze errors are predominantly slips (99.2%),** not timeouts—validating the Midas Touch problem in XR gaze interaction
3. **Adaptive UI (declutter) showed modest benefits** in gaze: slightly lower error rate (18.2% vs 19.1%)
...
if (has_gt) {
  results %>%
    ...
    format_gt_portfolio(
      ...
      footnote = "Error rate, throughput, movement time, and NASA-TLX workload scores. We measured subjective workload using NASA-TLX (0–100). It's widely used in HCI because it captures perceived effort and frustration that can increase even when average performance looks similar."
    )
} else {
  results %>%
...
This validates a core UX failure mode: gaze interaction suffers from intent ambiguity (looking to see vs. looking to select). The Midas Touch problem is real and measurable.
...
**What to look for:** Density clusters show where selection endpoints actually land relative to target center (0,0). The white dashed lines mark the target center. Bright yellow/orange indicates high density (many endpoints), dark purple indicates low density. Compare Adaptive vs Static to see if declutter affects endpoint clustering patterns. Hover over cells to see exact density values.
...
**What to look for:** Warmer colors indicate higher error rates. Hover over cells to see exact percentages. Compare Hand vs Gaze to see if certain screen regions show consistently higher errors—this could indicate UI layout issues or visual attention patterns.
...
### Why a Fitts' Law pointing task (and why TP)?

To compare hand vs gaze fairly, we used an ISO 9241-9`r tip_i("International standard for evaluating pointing devices. Defines Fitts' Law task structure, effective width calculation (We = 4.133 × SD), and Throughput metric.")` style Fitts' Law`r tip_i("Standard HCI benchmark for target acquisition. Difficulty (ID) increases with distance and decreases with target width. Widely used to compare input methods.")` pointing task — the standard HCI benchmark for target acquisition (selection). It's widely used because it isolates the core interaction loop ("move → acquire → confirm") while letting us systematically control difficulty via target size and distance. That makes it ideal for studying XR selection failure modes (e.g., gaze slips / Midas Touch) and testing UI assistance policies under time pressure. Selection is one of the highest-frequency actions in XR UI; benchmarking target acquisition gives direct insight into practical UI reliability.

Rather than looking at speed and errors separately, we report **Throughput (TP)**`r tip_i("Speed–accuracy efficiency metric: TP = IDe / MT. Higher TP means users acquire targets faster without sacrificing precision. Prevents misleading 'accuracy by slowing down' interpretations.")` as the primary performance metric. TP summarizes how efficiently users can acquire targets by combining **movement time** and **effective accuracy** into a single value (higher is better). In practice, TP helps avoid misleading conclusions like "faster, but much sloppier" or "more accurate, but only because users slowed down."

::: {.callout-tip}
**TP is a speed–accuracy efficiency score:** It rises when users get faster *without* sacrificing precision. Adaptations can "improve accuracy" by slowing people down; TP makes that tradeoff explicit.
:::

::: {.callout-note collapse="true"}
## Metric cheat sheet (optional)
...
\[
W_e = 4.133 \cdot SD(\text{endpoint error})
\]
...
TP=\frac{ID_e}{MT}
\]
Higher TP = better speed–accuracy efficiency.
:::

**Task:** Fitts' law pointing with varying target distances (A) and widths (W).  
**Design:** 2×2×2 within-subjects factorial (modality × UI mode × pressure)

**Conditions:** 
- Modality: Hand (mouse) vs. Gaze (simulated)
- UI Mode: Static vs. Adaptive (declutter`r tip_i("Declutter: adaptive UI policy that removes non-essential visual elements (e.g., background decorations, secondary buttons) to reduce visual noise and improve target visibility.")` for gaze; width inflation`r tip_i("Width inflation: adaptive UI policy that increases target width (width_scale_factor > 1.0) to make targets easier to acquire. Not activated in this dataset.")` for hand, but not activated)
- Pressure: Low (0) vs. High (1) cognitive load

**Participants:** N = `r if(!is.na(n_hand)) n_hand else "—"` hand (mouse users), N = `r if(!is.na(n_gaze)) n_gaze else "—"` gaze (mouse + trackpad users). Trackpad users excluded from hand modality due to device confound.

**Trials:** ~200 per participant, balanced across conditions using Williams sequences`r tip_i("Williams sequences: counterbalancing method that ensures each condition appears equally often in each position across participants. Reduces order effects.")`.

**Metrics:** Throughput (bits/s), Movement Time (s), Error Rate (%), NASA-TLX`r tip_i("NASA Task Load Index: 6 subscales (Mental, Physical, Temporal, Performance, Effort, Frustration). We report unweighted average (0–100). Higher = more workload.")` workload (6 subscales)

**Analysis:** Linear mixed-effects models`r tip_i("Mixed-effects models: statistical models with both fixed effects (conditions) and random effects (participant-level variation). Handles repeated measures and individual differences.")` (LMM) for continuous outcomes, generalized LMM`r tip_i("GLMM: Generalized Linear Mixed Model. Used for binary outcomes (like error rates). Extends LMM to handle non-normal distributions.")` (GLMM) for error rates, with random intercepts per participant. Type III ANOVA`r tip_i("Type III ANOVA: uses sum-to-zero contrasts, appropriate for unbalanced designs. Tests main effects and interactions after accounting for other terms.")` with sum-to-zero contrasts.

::: {.callout-note}
## Adaptive System: What Was Evaluable vs Not {#adaptive-system-evaluable}

**Gaze adaptive UI (declutter) was evaluated:** PolicyEngine activated declutter based on performance triggers (RT p75 thresholds, error bursts). Results show modest error reduction (18.2% vs 19.1%) but no significant throughput benefit.

**Hand adaptive UI (width inflation) did not activate:** PolicyEngine emitted 243 `inflate_width` actions (17 participants), but `width_scale_factor``r tip_i("width_scale_factor: multiplier applied to target width. 1.0 = no change, >1.0 = wider targets. Remained 1.0 throughout, indicating width inflation never activated.")` remained 1.0 throughout. Root cause: UI integration issue—policy actions were emitted but not applied to rendered targets. See [Appendix](#appendix) for full diagnostic analysis.

## Implications for XR Product Teams {#implications}

### For Gaze Interaction: Slip-Prevention Beats Declutter

The dominant gaze failure mode is slips (99.2%), not simple misses. Decluttering UI helps, but the bigger opportunity is **intent disambiguation**—confirmation mechanisms, stabilization, snapping, dwell tuning, and clear state feedback. Prioritize reducing the verification burden rather than just reducing visual chrome.

### For Hand Interaction: Instrumentation Caught Non-Activation

Hand width inflation did not activate in this dataset; instrumentation and validated logging caught this early. The feature was instrumented but not exercised, so its benefit is unconfirmed.
...
- **Hand adaptive UI not exercised:** Width inflation did not activate (width_scale_factor always 1.0), so hand UI-mode effects cannot be interpreted as adaptation effectiveness. Root cause: PolicyEngine emitted actions but UI integration did not apply them.
- **Unbalanced design:** Hand N=75 (mouse only), Gaze N=81 (mouse + trackpad). Type III ANOVA handles this appropriately.
- **Gaze simulation:** Gaze input uses physiologically-informed simulation, not eye tracking hardware.
:::

::: {.callout-tip}
## Lessons Learned

- **Device standardization matters:** Trackpad vs. mouse differences required exclusion of trackpad users from hand modality.
- **Policy integration critical:** PolicyEngine logic executed correctly, but UI integration failed to apply width scaling actions.
- **Gaze errors are slips, not timeouts:** 99.2% of gaze errors are slips (accidental activations), validating the Midas Touch problem. This suggests intent disambiguation (confirmation, stabilization) is more important than reducing UI chrome.
- **Adaptive UI benefits modest:** Declutter showed small error reduction in gaze, but throughput effects were minimal. Targeting the dominant failure mode (slips) may be more impactful than general UI simplification.
:::

## Appendix {#appendix}

<details>
<summary><strong>Technical Details & Statistical Analysis</strong> (Click to expand)</summary>
...
- **Error Rate:** `error ~ modality × ui_mode × pressure + (1 | pid)` (binomial GLMM)  
- **Movement Time:** `log(RT) ~ modality × ui_mode × pressure + (1 | pid)`  
- **NASA-TLX:** `TLX ~ modality × ui_mode × pressure + (1 | pid)`

All models use Type III ANOVA with sum-to-zero contrasts. Random intercepts per participant.

**Key Interpretation Rule:** UI-mode effects reflect declutter in gaze; width inflation not exercised in hand, so any hand UI-mode effects are treated as noise/non-causal.
...
**Estimated Marginal Means (EMMs) with Holm-adjusted pairwise contrasts** for ui_mode within each pressure level.
  ...
  if (has_gt && nrow(width_diag) > 0) {
    width_diag %>%
      ...
      tab_header(
        ...
        subtitle = "width_scale_factor == 1.0 throughout (no activation)"
      ) %>%
      opt_table_font(font = "system-ui, -apple-system") %>%
      opt_table_lines(extent = "none") %>%
      tab_options(
...
- Width inflation never activated: 0 trials with `width_scale_factor != 1.0`
- PolicyEngine emitted 243 `inflate_width` actions (17 participants), but UI integration did not apply them
- Root cause: UI integration issue—policy actions were emitted but not applied to rendered targets
...
- **Telemetry Schema:** Per-trial logging includes: trial metadata (modality, ui_mode, pressure), performance metrics (RT, correctness, error type), Fitts' parameters (A, W, ID), endpoint coordinates, movement samples (trajectory), display state (zoom, fullscreen, tab visibility), policy state (adaptation triggers, actions)
- **QC Signals:** Zoom level, fullscreen mode, tab hidden duration, focus/blur events, RT bounds, input device type
- **Deployment:** Vercel with automated CI/CD

**Architecture:**

```{r architecture-placeholder}
if (file.exists("docs/assets/case_study/portfolio/architecture.png")) {
  knitr::include_graphics("docs/assets/case_study/portfolio/architecture.png", dpi = 200)
} else {
  cat("Architecture diagram placeholder: `docs/assets/case_study/portfolio/architecture.png`\n")
}
```

**Telemetry Schema:**

```{r telemetry-schema-placeholder}
cat("Telemetry schema table placeholder (key fields: trial metadata, performance metrics, Fitts' parameters, endpoint data, movement samples, display state, policy state)\n")
```

### Gaze proxy (psychophysics + calibration + reliability guards)

Gaze input uses a physiologically-informed simulation rather than eye tracking hardware. This approach allows controlled comparison of hand vs gaze interaction patterns while maintaining ecological validity for XR selection tasks.

**Psychophysics model:**

- **Gaussian jitter** (σ ≈ 1–2° visual angle): Simulates natural eye movement variability. Real eye tracking has inherent noise; Gaussian jitter captures this uncertainty in gaze position estimation.
- **Saccadic suppression**: Temporary input masking during rapid eye movements (saccades). Prevents false selections during eye movements, mimicking how real eye trackers suppress input during saccades.
- **Latency injection** (50–150ms delay): Approximates real-world eye tracking system delays. Commercial eye trackers have processing latency; injected delay makes simulation more realistic.

**Calibration:**

- **Pixels-per-degree conversion**: Based on screen dimensions and viewing distance assumptions. Converts screen pixels to visual angle degrees, enabling consistent difficulty across different display sizes.

**Reliability guards (QC gates):**

- **Zoom level check**: Must be 100% (prevents scaling artifacts that would distort target sizes)
- **Fullscreen mode**: Required (ensures consistent viewport and eliminates browser chrome interference)
- **Tab visibility**: Hidden duration <500ms threshold (filters trials where user switched tabs)
- **Focus/blur events**: Zero tolerance (ensures window had focus throughout trial)
- **RT bounds**: 150–6000ms (filters non-physiological responses: too fast = accidental clicks, too slow = disengagement)

These guards ensure data quality and prevent confounds from display state or user disengagement.

### Planned: LBA modeling

**What LBA adds:**

Linear Ballistic Accumulator (LBA) modeling provides latent parameter interpretation: **drift rate** (information accumulation speed), **threshold** (decision boundary), and **non-decision time** (NDT, encoding + motor execution). This decomposes RT into cognitive components, revealing how modality and pressure affect decision-making processes.

**Inputs needed:**

Trial-level RT + correctness data, separated by modality × pressure conditions.

**Planned output:**

Forest plot of LBA parameters (drift, threshold, NDT) by modality × pressure, showing how gaze vs hand differ in information accumulation and decision thresholds.
