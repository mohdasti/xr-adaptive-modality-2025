...
Sample size was determined through a combination of power analysis and Williams design constraints. For a 2 × 2 × 2 within-subjects design with an expected medium-to-large effect size (Cohen's $d = 0.60$), power of 0.80, and $\alpha = 0.05$ (two-tailed), a minimum of $N = 24$ participants would be required for a simple repeated-measures design. However, to ensure proper counterbalancing with the Williams Latin square arrangement, the sample size must be a multiple of the number of treatment sequences. For our 2 × 2 × 2 factorial design, we require 8 distinct counterbalancing sequences. We selected $N = 48$ participants, which allows for 6 complete Williams blocks (8 participants per block), ensuring balanced representation across all treatment orders while providing increased statistical power ($\approx 0.95$ for $d = 0.60$ in a within-subjects design) and robustness to potential data exclusions.

### Recruitment

We recruited $N=48$ participants (target: balanced gender distribution, age range 18-35) from the university community. All participants had normal or corrected-to-normal vision and no known motor impairments. The study was approved by the Institutional Review Board (IRB) and all participants provided informed consent.

### Participant Exclusions

Seven participants (P002, P003, P007, P008, P015, P039, P040) were excluded from the main 2×2×2 factorial analysis due to a data logging error that incorrectly recorded all trials as `pressure = 1` regardless of block condition. This error was identified during a comprehensive data audit on December 8, 2025, and was caused by passing the pressure value (always 1.0) instead of the pressure condition boolean (`pressureEnabled`) to the logging function. The bug was fixed in commit `04758db`, and seven replacement participants (P049-P055) will be collected to maintain the planned target sample size of $N=48$ participants with complete data across all experimental conditions. The affected participants' data were retained for exploratory analyses but excluded from the primary factorial analysis to ensure all factors had complete data across all levels. All analyses in the current interim dataset use $N=23$ participants with complete data.
...
We report interim results from the current dataset (N=23 participants with complete factorial data; broader analyses include up to N=81 where applicable). Full confirmatory analyses with mixed-effects models will be reported upon completion of the target sample (N=48).
...
The gaze adaptive manipulation (declutter) was the only adaptation that executed in this dataset. Gaze error rate was modestly lower in adaptive (18.2%) than static (19.1%) mode. The declutter mechanism reduced timeouts (1.18% → 0.42%) but did not materially reduce slips (98.8% → 99.6%), suggesting that decluttering mitigates distraction-driven hesitation but does not address the core intent disambiguation problem. At N=23, this effect is directional; confirmatory tests will be reported at N=48.

## Subjective Workload (RQ2)

NASA-TLX scores (0–100) were higher for gaze than hand across all subscales (@tbl-tlx). Overall workload (unweighted mean of six subscales) was 40.4 [37.0, 43.8] for hand and 47.0 [43.9, 50.2] for gaze. For the RQ2-specified subscales, **Physical Demand** was 34.3 [30.8, 37.8] (hand) vs. 41.9 [38.4, 45.4] (gaze), and **Frustration** was 32.2 [28.5, 35.9] (hand) vs. 43.2 [39.5, 46.9] (gaze). Gaze interaction imposed higher perceived physical demand and frustration, consistent with the Midas Touch burden and the need for sustained dwell/verification.
...
::: {#tbl-lba-params}
| Condition | t0 (NDT) | Drift Base | ID Slope | Threshold Int. | Pressure Slope |
|:---------|:--------:|-----------:|:--------:|---------------:|----------------:|
| Hand – Static | −2.85 | 5.03 | −0.93 | 4.69 | 0.06 |
| Hand – Adaptive | −3.01 | 5.03 | −0.93 | 4.69 | 0.06 |
| Gaze – Static | −1.41 | 5.03 | −0.93 | 4.69 | 0.06 |
| Gaze – Adaptive | −0.97 | 5.03 | −0.93 | 4.69 | 0.06 |

: LBA parameter estimates by modality and UI mode. Parameters are in log-space (softplus transformed). t0: non-decision time; ID slope: effect of difficulty on drift (negative = harder trials reduce drift); Pressure slope: effect of time pressure on threshold.
:::

### Interpretation

**Modality effect on NDT:** Gaze conditions show higher t0 values (less negative: −1.41 to −0.97) than hand conditions (−3.01 to −2.85), indicating longer verification-phase duration for gaze interaction. This aligns with the Midas Touch problem: gaze requires additional time for intent disambiguation before selection.

**UI mode effect:** Adaptive UI shows different t0 patterns, particularly for gaze (static: −1.41, adaptive: −0.97). The higher t0 in gaze-adaptive suggests that declutter may alter verification timing, though the direction of the effect warrants further investigation with the full sample.

**Difficulty and pressure:** The negative ID slope (−0.93) confirms that harder trials reduce drift rate, consistent with Fitts's Law. The positive pressure slope (0.06) indicates a speed–accuracy tradeoff: higher time pressure increases the decision threshold.

### Convergence Diagnostics

@fig-lba-trace shows MCMC trace plots for key parameters. The drift-rate slope (vc_slope_mu) and threshold slope (gap_slope_mu) exhibit unimodal posteriors and well-mixed chains. The t0_mu parameter shows multiple modes across the four condition cells, reflecting the hierarchical structure; R-hat remained at 1.0 for all parameters, and ESS exceeded 2,000.

![MCMC trace plots for key LBA parameters. Left panels: posterior densities; right panels: sampling chains. Parameters vc_slope_mu and gap_slope_mu show unimodal posteriors and good mixing; t0_mu reflects the four condition-specific estimates.](../../outputs/LBA/lba_trace_plot.png){#fig-lba-trace width=90%}
