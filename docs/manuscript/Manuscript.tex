...
dataset, width scaling did not activate due to policy configuration
requiring pressure-enabled blocks combined with participants not meeting
performance degradation thresholds. Width scaling functionality is
implemented and logged for analysis in future data collection.
...
``expanding targets'' technique (McGuffin \& Balakrishnan, 2005).
However, in the current dataset, width scaling did not activate due to
policy configuration constraints (\texttt{pressure\_only:\ true}
requiring pressure-enabled blocks) and performance thresholds not being
met. Width scaling functionality is implemented and will be analyzed in
future data collection runs.
...
\subsubsection{Participant Exclusions}\label{participant-exclusions}

Seven participants (P002, P003, P007, P008, P015, P039, P040) were
excluded from the main 2×2×2 factorial analysis due to a data logging
error that incorrectly recorded all trials as \texttt{pressure\ =\ 1}
regardless of block condition. This error was identified during a
comprehensive data audit on December 8, 2025, and was caused by passing
the pressure value (always 1.0) instead of the pressure condition
boolean (\texttt{pressureEnabled}) to the logging function. The bug was
fixed in commit \texttt{04758db}, and seven replacement participants
(P049-P055) will be collected to maintain the planned target sample size
of \(N=48\) participants with complete data across all experimental
conditions. The affected participants' data were retained for
exploratory analyses but excluded from the primary factorial analysis to
ensure all factors had complete data across all levels. All analyses in
the current interim dataset use \(N=23\) participants with complete
data.
...
boundary separation parameters (Lerche et al., 2017). We adopted a
\textbf{Hybrid Analysis} framework to disentangle ``motor'' performance
from ``decision'' caution: (1) \textbf{Macro-Performance (Fitts'
Law)}---we modeled the overall throughput (bits/s) and the regression
slope (\(b\)) to quantify the global cost of the gaze simulation lag,
following standard ISO 9241-9 throughput calculations; (2) \textbf{Path
Quality (Control Theory)}---we calculated pre-computed submovement
counts (velocity peaks detected in real-time) and target re-entries to
quantify the ``cost of control'' (how often the user had to correct
their trajectory due to simulated lag and ``saccadic blindness''),
drawing on the Optimized Submovement Model (Meyer et al., 1988). Full
trajectory-based velocity profile decomposition and primary
vs.~corrective movement identification will be conducted once the sample
size reaches N=48; (3) \textbf{Decision Verification (LBA)}---we used
the Linear Ballistic Accumulator (LBA) model (Brown \& Heathcote, 2008)
to analyze the verification phase (time from first target entry to final
selection). LBA is robust to low error rates and allows estimation of
non-decision time (NDT), drift rate, and decision threshold. We fit a
...
with complete factorial data; broader analyses include up to N=81 where
applicable). Full confirmatory analyses with mixed-effects models will
be reported upon completion of the target sample (N=48).
...
Movement time was shorter for hand (1.09 s) than gaze (1.19 s). The
modality effect on throughput and error rate is large and robust at this
sample size. Figure~\ref{fig-throughput} and Figure~\ref{fig-error-rate}
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{../assets/case_study/throughput.png}

}

\caption{\label{fig-throughput}Throughput (bits/s) by modality. Hand
produced higher throughput than gaze. Error bars show 95\% CI.}
...
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{../assets/case_study/error_rate.png}

}

\caption{\label{fig-error-rate}Error rate (\%) by modality. Gaze showed
substantially higher error rate than hand. Error bars show 95\% CI.}

\end{figure}%

\subsection{Error Profile: Midas Touch
Confirmation}\label{error-profile-midas-touch-confirmation}
...
4.3\% were timeouts. This asymmetry provides empirical confirmation of
the Midas Touch problem: gaze interaction fails primarily due to intent
ambiguity (looking to see vs.~looking to select), whereas hand
interaction fails due to spatial targeting errors.
...
99.6\%), suggesting that decluttering mitigates distraction-driven
hesitation but does not address the core intent disambiguation problem.
At N=23, this effect is directional; confirmatory tests will be reported
at N=48.
...
vs.~43.2 {[}39.5, 46.9{]} (gaze). Gaze interaction imposed higher
perceived physical demand and frustration, consistent with the Midas
Touch burden and the need for sustained dwell/verification.
...
Figure~\ref{fig-tlx-stacked} shows the subscale composition.
...
\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{../assets/case_study/tlx_stacked.png}

}

\caption{\label{fig-tlx-stacked}NASA-TLX subscale composition by
modality. Stacked bars show contribution of each subscale (Mental,
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1333}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.2133}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.2267}}@{}}
\caption{LBA parameter estimates by modality and UI mode. Parameters are
in log-space (softplus transformed). t0: non-decision time; ID slope:
effect of difficulty on drift (negative = harder trials reduce drift);
Pressure slope: effect of time pressure on
threshold.}\label{tbl-lba-params}\tabularnewline
\toprule\noalign{}
...
indicating longer verification-phase duration for gaze interaction. This
aligns with the Midas Touch problem: gaze requires additional time for
intent disambiguation before selection.
...
gaze-adaptive suggests that declutter may alter verification timing,
though the direction of the effect warrants further investigation with
the full sample.

\textbf{Difficulty and pressure:} The negative ID slope (−0.93) confirms
that harder trials reduce drift rate, consistent with Fitts's Law. The
positive pressure slope (0.06) indicates a speed--accuracy tradeoff:
higher time pressure increases the decision threshold.

\subsubsection{Convergence Diagnostics}\label{convergence-diagnostics}

Figure~\ref{fig-lba-trace} shows MCMC trace plots for key parameters.
The drift-rate slope (vc\_slope\_mu) and threshold slope
(gap\_slope\_mu) exhibit unimodal posteriors and well-mixed chains. The
t0\_mu parameter shows multiple modes across the four condition cells,
reflecting the hierarchical structure; R-hat remained at 1.0 for all
parameters, and ESS exceeded 2,000.
...
Left panels: posterior densities; right panels: sampling chains.
Parameters vc\_slope\_mu and gap\_slope\_mu show unimodal posteriors and
good mixing; t0\_mu reflects the four condition-specific estimates.}
