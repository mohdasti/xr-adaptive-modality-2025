...
if ("trajectory" %in% names(df_raw) && "A" %in% names(df_raw)) {
  df_drag <- df_raw %>%
    ...
    mutate(
      ...
      drag_distance = if ("drag_distance" %in% names(.)) {
        case_when(
          is.na(drag_distance) | drag_distance == 0 ~ path_length_computed,
          is.numeric(drag_distance) & drag_distance > 0 ~ drag_distance,
          TRUE ~ path_length_computed
        )
      } else {
...
if (exists("df_drag") && nrow(df_drag) > 0 && "ID" %in% names(df_drag)) {
  df_binned <- df_drag %>%
    ...
    mutate(
      ID_bin = cut(ID, 
                   breaks = quantile(ID, probs = seq(0, 1, 0.25), na.rm = TRUE, names = FALSE),
                   include.lowest = TRUE,
  ...
  if (nrow(df_binned) > 0 && sum(df_binned$n) > 0) {
    p3 <- ggplot(df_binned, aes(x = ID_bin, y = mean_ratio, group = ui_mode, color = ui_mode)) +
      ...
      scale_color_manual(values = custom_palette_2, labels = c("Static", "Adaptive")) +
      labs(
        x = "Index of Difficulty (binned by quartiles)",
