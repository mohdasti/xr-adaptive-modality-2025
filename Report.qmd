...
# Note: Type III ANOVA is necessary for unbalanced designs (hand and gaze N differ due to input device exclusion)
...
# --- HELPER: INPUT DEVICE SAMPLE SIZE COMPUTATION ---
# Compute actual sample sizes for unbalanced design reporting
compute_input_device_stats <- function(data) {
  if (!"input_device" %in% names(data)) {
    return(list(
      hand_n = 0,
      gaze_n = 0,
      mouse_n = 0,
      trackpad_n = 0,
      trackpad_hand_trials = 0,
      trackpad_gaze_trials = 0
    ))
  }
  
  # Get unique participants by input device
  device_pids <- data %>%
    filter(!is.na(input_device)) %>%
    distinct(pid, input_device, .keep_all = TRUE)
  
  mouse_pids <- device_pids %>% filter(input_device == "mouse") %>% pull(pid)
  trackpad_pids <- device_pids %>% filter(input_device == "trackpad") %>% pull(pid)
  
  # Hand modality: only mouse users
  hand_n <- data %>%
    filter(modality == "hand", pid %in% mouse_pids) %>%
    distinct(pid) %>%
    n_distinct()
  
  # Gaze modality: all users (mouse + trackpad)
  gaze_n <- data %>%
    filter(modality == "gaze") %>%
    distinct(pid) %>%
    n_distinct()
  
  # Trackpad users' trial counts (for exclusion impact)
  trackpad_hand_trials <- if (length(trackpad_pids) > 0) {
    data %>% filter(pid %in% trackpad_pids, modality == "hand") %>% nrow()
  } else { 0 }
  
  trackpad_gaze_trials <- if (length(trackpad_pids) > 0) {
    data %>% filter(pid %in% trackpad_pids, modality == "gaze") %>% nrow()
  } else { 0 }
  
  return(list(
    hand_n = hand_n,
    gaze_n = gaze_n,
    mouse_n = length(mouse_pids),
    trackpad_n = length(trackpad_pids),
    trackpad_hand_trials = trackpad_hand_trials,
    trackpad_gaze_trials = trackpad_gaze_trials
  ))
}
...
if ("input_device" %in% names(df)) {
  ...
  cat("\n### Input Device Distribution (Before Exclusion)\n\n")
  ...
  if (length(trackpad_pids) > 0) {
    ...
    cat("**Input Device Exclusion Applied:**\n")
    cat("- Trackpad users: N = ", length(trackpad_pids), " participants\n")
    cat("- Hand trials excluded: ", trackpad_hand_trials, " (device confound for hand modality)\n")
  ...
  cat("**Exclusion rationale:** Hand modality requires device-dependent motor control, so we standardize device (mouse only). Gaze modality uses physiologically-informed simulation that normalizes input, so device doesn't matter after simulation. This maximizes data utilization while maintaining validity.\n\n")
  ...
  # Also update df_all_trials for error rate analysis
  if (exists("df_all_trials")) {
...
```{r follow-up-models}
# Throughput follow-ups: Gaze-only (ui_mode * pressure) and Hand-only (pressure only)
if (exists("df_tp_model") && nrow(df_tp_model) > 0) {
  df_tp_gaze <- df_tp_model %>% filter(modality == "gaze")
  df_tp_hand <- df_tp_model %>% filter(modality == "hand")
  
  # Gaze-only TP model
  model_tp_gaze <- NULL
  emm_tp_gaze <- NULL
  pairs_tp_gaze <- NULL
  if (nrow(df_tp_gaze) > 0 && n_distinct(df_tp_gaze$pid) >= 2) {
    model_tp_gaze <- tryCatch({
      lmer(TP ~ ui_mode * pressure + (1|pid), data = df_tp_gaze, 
           REML = FALSE, control = lmerControl(optimizer = "bobyqa"))
    }, error = function(e) NULL)
    
    if (!is.null(model_tp_gaze)) {
      emm_tp_gaze <- tryCatch({
        emmeans(model_tp_gaze, ~ ui_mode * pressure)
      }, error = function(e) NULL)
      
      if (!is.null(emm_tp_gaze)) {
        pairs_tp_gaze <- tryCatch({
          pairs(emm_tp_gaze, adjust = "holm")
        }, error = function(e) NULL)
      }
    }
  }
  
  # Hand-only TP model (pressure only)
  model_tp_hand <- NULL
  emm_tp_hand <- NULL
  pairs_tp_hand <- NULL
  if (nrow(df_tp_hand) > 0 && n_distinct(df_tp_hand$pid) >= 2) {
    model_tp_hand <- tryCatch({
      lmer(TP ~ pressure + (1|pid), data = df_tp_hand, 
           REML = FALSE, control = lmerControl(optimizer = "bobyqa"))
    }, error = function(e) NULL)
    
    if (!is.null(model_tp_hand)) {
      emm_tp_hand <- tryCatch({
        emmeans(model_tp_hand, ~ pressure)
      }, error = function(e) NULL)
      
      if (!is.null(emm_tp_hand)) {
        pairs_tp_hand <- tryCatch({
          pairs(emm_tp_hand, adjust = "holm")
        }, error = function(e) NULL)
      }
    }
  }
...
# RT follow-ups: Gaze-only (ui_mode * pressure) and Hand-only (pressure only)
if (exists("df_factorial") && nrow(df_factorial) > 0) {
  df_rt_gaze <- df_factorial %>% filter(modality == "gaze")
  df_rt_hand <- df_factorial %>% filter(modality == "hand")
  
  # Gaze-only RT model
  model_rt_gaze <- NULL
  emm_rt_gaze <- NULL
  pairs_rt_gaze <- NULL
  if (nrow(df_rt_gaze) > 0 && n_distinct(df_rt_gaze$pid) >= 2) {
    model_rt_gaze <- tryCatch({
      lmer(log_rt ~ ui_mode * pressure + (1|pid), data = df_rt_gaze, 
           REML = FALSE, control = lmerControl(optimizer = "bobyqa"))
    }, error = function(e) NULL)
    
    if (!is.null(model_rt_gaze)) {
      emm_rt_gaze <- tryCatch({
        emmeans(model_rt_gaze, ~ ui_mode * pressure, type = "response")
      }, error = function(e) NULL)
      
      if (!is.null(emm_rt_gaze)) {
        pairs_rt_gaze <- tryCatch({
          pairs(emm_rt_gaze, adjust = "holm")
        }, error = function(e) NULL)
      }
    }
  }
  
  # Hand-only RT model (pressure only)
  model_rt_hand <- NULL
  emm_rt_hand <- NULL
  pairs_rt_hand <- NULL
  if (nrow(df_rt_hand) > 0 && n_distinct(df_rt_hand$pid) >= 2) {
    model_rt_hand <- tryCatch({
      lmer(log_rt ~ pressure + (1|pid), data = df_rt_hand, 
           REML = FALSE, control = lmerControl(optimizer = "bobyqa"))
    }, error = function(e) NULL)
    
    if (!is.null(model_rt_hand)) {
      emm_rt_hand <- tryCatch({
        emmeans(model_rt_hand, ~ pressure, type = "response")
      }, error = function(e) NULL)
      
      if (!is.null(emm_rt_hand)) {
        pairs_rt_hand <- tryCatch({
          pairs(emm_rt_hand, adjust = "holm")
        }, error = function(e) NULL)
      }
    }
  }
...
# Error follow-ups: Gaze-only (ui_mode * pressure) and Hand-only (pressure only, if variance sufficient)
if (exists("df_errors_factorial") && nrow(df_errors_factorial) > 0) {
  df_err_gaze <- df_errors_factorial %>% filter(modality == "gaze")
  df_err_hand <- df_errors_factorial %>% filter(modality == "hand")
  
  # Gaze-only Error model
  model_err_gaze <- NULL
  emm_err_gaze <- NULL
  pairs_err_gaze <- NULL
  if (nrow(df_err_gaze) > 0 && n_distinct(df_err_gaze$pid) >= 2) {
    error_rate_gaze <- mean(df_err_gaze$error, na.rm = TRUE)
    if (error_rate_gaze >= 0.01 && error_rate_gaze <= 0.99) {
      model_err_gaze <- tryCatch({
        glmer(error ~ ui_mode * pressure + (1|pid), data = df_err_gaze, 
              family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
      }, error = function(e) NULL)
      
      if (!is.null(model_err_gaze)) {
        emm_err_gaze <- tryCatch({
          emmeans(model_err_gaze, ~ ui_mode * pressure, type = "response")
        }, error = function(e) NULL)
        
        if (!is.null(emm_err_gaze)) {
          pairs_err_gaze <- tryCatch({
            pairs(emm_err_gaze, adjust = "holm")
          }, error = function(e) NULL)
        }
      }
    }
  ...
  # Hand-only Error model (pressure only, if variance sufficient)
  model_err_hand <- NULL
  emm_err_hand <- NULL
  pairs_err_hand <- NULL
  if (nrow(df_err_hand) > 0 && n_distinct(df_err_hand$pid) >= 2) {
    error_rate_hand <- mean(df_err_hand$error, na.rm = TRUE)
    if (error_rate_hand >= 0.01 && error_rate_hand <= 0.99) {
      model_err_hand <- tryCatch({
        glmer(error ~ pressure + (1|pid), data = df_err_hand, 
              family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
      }, error = function(e) NULL)
      
      if (!is.null(model_err_hand)) {
        emm_err_hand <- tryCatch({
          emmeans(model_err_hand, ~ pressure, type = "response")
        }, error = function(e) NULL)
        
        if (!is.null(emm_err_hand)) {
          pairs_err_hand <- tryCatch({
            pairs(emm_err_hand, adjust = "holm")
          }, error = function(e) NULL)
        }
      }
    } else {
      cat("⚠ **Hand error model skipped:** Insufficient variance (error rate = ", round(100 * error_rate_hand, 1), "%). Descriptive only.\n\n")
    }
  }
}
```
...
<!-- [CORE] Required: QC metrics, exclusion rationale, sample size reporting -->
...
<!-- ============================================ -->
<!-- SECTION ORGANIZATION: CORE vs SUPPLEMENT    -->
<!-- ============================================ -->
<!-- This report contains both CORE sections (addressing pre-registered RQs/Hs) -->
<!-- and SUPPLEMENT sections (exploratory/descriptive analyses).                -->
<!-- For case study writing, focus on CORE sections (3, 4, 6, 8).              -->
<!-- For preprint, CORE sections go in main paper; SUPPLEMENT in supplement.   -->
<!-- ============================================ -->


# 1. Executive Summary

<!-- [CORE] Critical: Overview of key findings -->
...
**Important Note:** In this dataset, hand 'adaptive' did not change target width (width_scale_factor remained 1.0), so hand UI mode is treated as not exercised. Adaptation conclusions therefore focus on gaze declutter, which was evaluated and operated as intended. Hand models exclude UI mode by design, and hand descriptive reporting collapses across ui_mode (static+adaptive) to report pressure and modality effects only.

# Results Snapshot (N = `r qc_participants_included`)

```{r results-snapshot}
# Gaze-only: adaptive - static contrasts within each pressure level + overall
gaze_snapshot_by_pressure <- df_pid_cond %>%
  filter(modality == "gaze") %>%
  group_by(modality, ui_mode, pressure) %>%
  summarise(
    tp = mean(tp_mean, na.rm = TRUE),
  ...
  select(modality, pressure, tp_diff_adapt_static, rt_diff_adapt_static, err_diff_adapt_static)

# Gaze-only: overall (collapsed across pressure)
gaze_snapshot_overall <- df_pid_cond %>%
  filter(modality == "gaze") %>%
...
gaze_snapshot <- bind_rows(gaze_snapshot_by_pressure, gaze_snapshot_overall)
...
gaze_snapshot %>%
  kable(caption = "Gaze contrasts: adaptive - static (by pressure level + overall)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)

cat("\n### Hand: UI-Mode Manipulation Not Exercised\n\n")
cat("**Note:** Hand UI-mode manipulation not exercised (width_scale_factor always 1.0), so adaptive vs static comparisons are non-inferential/no-manipulation. Hand analyses focus on pressure and modality effects only.\n\n")

# Show hand pressure effect for reference
hand_pressure_snapshot <- df_pid_cond %>%
  filter(modality == "hand") %>%
  group_by(modality, pressure) %>%
  summarise(
    tp = mean(tp_mean, na.rm = TRUE),
    rt = mean(rt_mean, na.rm = TRUE),
    err = mean(error_rate, na.rm = TRUE),
  ...
  pivot_wider(names_from = pressure, values_from = c(tp, rt, err)) %>%
  mutate(
    tp_diff_p1_p0 = tp_1 - tp_0,
    rt_diff_p1_p0 = rt_1 - rt_0,
    err_diff_p1_p0 = err_1 - err_0
  ) %>%
  select(modality, tp_diff_p1_p0, rt_diff_p1_p0, err_diff_p1_p0)

hand_pressure_snapshot %>%
  kable(caption = "Hand: pressure ON - pressure OFF (UI mode collapsed)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
...
cat("\n*Note:* Hand width inflation did not activate; all recorded `width_scale_factor` values equal 1.0. Root cause analysis indicates policy actions were emitted but not applied to UI (see replay diagnostics).\n\n")
```
...
<!-- [CORE - PRIMARY] RQ1: Core performance metric, pre-registered hypothesis -->

**Research Question:** Does the Adaptive UI improve performance (Throughput) compared to Static, especially for Gaze?

**Sample Size:** `r get_n_string(df_iso %>% filter(modality == "hand"))` participants for hand modality (mouse users only), `r get_n_string(df_iso %>% filter(modality == "gaze"))` participants for gaze modality (mouse + trackpad users). See Data Quality section for input device exclusion rationale.
...
if (exists("model_tp") && !is.null(model_tp)) {
  ...
  # Prominent interpretation rule
  cat("\n**⚠ Interpretation Rule:** UI-mode effects in the omnibus model reflect declutter in gaze (the adaptation that operated). Width inflation did not exercise in hand (width_scale_factor always 1.0), so any hand UI-mode effects are treated as noise/non-causal, not adaptation effectiveness.\n\n")
  ...
  if (nrow(ui_mode_row) > 0 && !is.na(ui_mode_row$`F value`[1])) {
    ...
    cat("**UI Mode Effect (Omnibus):** The main effect of UI mode was ",
    ...
    cat("**Note:** This omnibus UI mode effect is diluted by the fact that HAND width inflation did not execute (width_scale_factor always 1), so UI mode is not interpretable as an adaptive manipulation for hand. The interpretable test of adaptation comes from the **gaze-only UI Mode × Pressure follow-up model** below.\n\n")
  }
  
  # Targeted follow-up (pre-planned due to non-exercised hand adaptation)
  cat("\n#### Targeted Follow-up: Gaze-Only Model (Pre-planned Due to Non-Exercised Hand Adaptation)\n\n")
  cat("**Rationale:** Hand width inflation did not activate (width_scale_factor always 1.0), so hand UI mode is treated as not exercised. The gaze-only model isolates the adaptation that actually operated (declutter).\n\n")
  
  # Gaze-only follow-up results
  if (exists("model_tp_gaze") && !is.null(model_tp_gaze) && exists("emm_tp_gaze") && !is.null(emm_tp_gaze)) {
    cat("##### Gaze-Only Model: UI Mode × Pressure (Primary Adaptive Test)\n\n")
    
    # ANOVA for gaze-only model
    anova_tp_gaze <- anova(model_tp_gaze, type = "III")
    print(anova_tp_gaze)
    cat("\n")
    
    # EMMs for gaze-only
    emm_tp_gaze_df <- as.data.frame(emm_tp_gaze)
    ci_lower <- if ("lower.CL" %in% names(emm_tp_gaze_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_tp_gaze_df)) "asymp.LCL" else NULL
    ci_upper <- if ("upper.CL" %in% names(emm_tp_gaze_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_tp_gaze_df)) "asymp.UCL" else NULL
    
    cat("**Estimated Marginal Means (Gaze-only):**\n")
    emm_tp_gaze_table <- emm_tp_gaze_df %>%
      mutate(
      ...
      select(`UI Mode`, Pressure, `Mean TP (bits/s)`, `95% CI Lower`, `95% CI Upper`)
    print(knitr::kable(emm_tp_gaze_table, caption = "Estimated Marginal Means for Throughput: Gaze-only"))
    cat("\n")
    
    # Key contrasts
    if (exists("pairs_tp_gaze") && !is.null(pairs_tp_gaze)) {
      cat("**Key Contrasts (Gaze-only, Holm-adjusted):**\n")
      pairs_tp_gaze_df <- as.data.frame(pairs_tp_gaze)
      # Focus on adaptive vs static under pressure ON and OFF
      key_contrasts <- pairs_tp_gaze_df %>%
        filter(grepl("adaptive.*static|static.*adaptive", contrast, ignore.case = TRUE))
      if (nrow(key_contrasts) > 0) {
        print(knitr::kable(key_contrasts %>% head(6), digits = 3))
      }
      cat("\n")
    }
  }
  
  # Hand-specific reporting: UI-mode not exercised
  cat("\n#### Hand: UI-Mode Not Exercised\n\n")
  cat("**Manipulation Check:**\n\n")
  
  # Check width_scale_factor for hand/adaptive/pressure=1
  if ("width_scale_factor" %in% names(df_raw)) {
    df_hand_adapt_p1_check <- df_raw %>%
      filter(
    ...
    if (nrow(df_hand_adapt_p1_check) > 0) {
      n_trials_check <- nrow(df_hand_adapt_p1_check)
      n_scaled_check <- sum(df_hand_adapt_p1_check$width_scale_factor != 1, na.rm = TRUE)
      pct_scaled_check <- 100 * n_scaled_check / n_trials_check
      
      manip_check_table <- data.frame(
        Condition = "Hand/Adaptive/Pressure=1",
        N_Trials = n_trials_check,
        N_With_Scaling = n_scaled_check,
        Pct_With_Scaling = round(pct_scaled_check, 2),
        Mean_Width_Scale = round(mean(df_hand_adapt_p1_check$width_scale_factor, na.rm = TRUE), 3)
      )
      
      print(knitr::kable(manip_check_table, caption = "Manipulation Check: Width Scaling in Hand/Adaptive/Pressure=1"))
      cat("\n")
      
      if (n_scaled_check == 0) {
        cat("**Conclusion:** H3/RQ3-hand cannot be tested in this dataset. Width inflation did not activate (width_scale_factor always 1.0), so hand UI-mode effects are not interpretable as adaptation effectiveness.\n\n")
      }
    }
  }
  
  # Hand-only follow-up results (pressure effect only)
  if (exists("model_tp_hand") && !is.null(model_tp_hand) && exists("emm_tp_hand") && !is.null(emm_tp_hand)) {
    cat("#### Hand-Only Follow-up: Pressure Effect\n\n")
    cat("*Note:* UI mode is excluded from hand models by design because width scaling did not execute.\n\n")
    
    # ANOVA for hand-only model
    anova_tp_hand <- anova(model_tp_hand, type = "III")
    print(anova_tp_hand)
    cat("\n")
    
    # EMMs for hand-only
    emm_tp_hand_df <- as.data.frame(emm_tp_hand)
    ci_lower <- if ("lower.CL" %in% names(emm_tp_hand_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_tp_hand_df)) "asymp.LCL" else NULL
    ci_upper <- if ("upper.CL" %in% names(emm_tp_hand_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_tp_hand_df)) "asymp.UCL" else NULL
    
    cat("**Estimated Marginal Means (Hand-only):**\n")
    emm_tp_hand_table <- emm_tp_hand_df %>%
      mutate(
        Pressure = as.character(pressure),
        `Mean TP (bits/s)` = round(emmean, 2),
        `95% CI Lower` = if (!is.null(ci_lower)) round(.data[[ci_lower]], 2) else NA_real_,
      ...
      select(Pressure, `Mean TP (bits/s)`, `95% CI Lower`, `95% CI Upper`)
    print(knitr::kable(emm_tp_hand_table, caption = "Estimated Marginal Means for Throughput: Hand-only (pressure effect)"))
    
    if (exists("pairs_tp_hand") && !is.null(pairs_tp_hand)) {
      cat("\n**Pressure Contrast (Hand-only, Holm-adjusted):**\n")
      pairs_tp_hand_df <- as.data.frame(pairs_tp_hand)
      print(knitr::kable(pairs_tp_hand_df, digits = 3))
    }
    cat("\n")
  }
  ...
  cat("**Omnibus takeaway:** The 2×2×2 factorial model shows a large modality main effect (hand > gaze) dominates throughput differences, with UI mode effects diluted by hand non-manipulation.\n\n")
  
  if (exists("model_tp_gaze") && !is.null(model_tp_gaze)) {
    cat("**Gaze-only takeaway:** The gaze-only UI Mode × Pressure model is the primary adaptation effectiveness test. Declutter effects in gaze are evaluable here.\n\n")
  }
  
  if (exists("model_tp_hand") && !is.null(model_tp_hand)) {
    cat("**Hand-only takeaway:** Hand models exclude UI mode by design because width_scale_factor never deviated from 1.0 (no manipulation occurred). Pressure effects are evaluable in hand, but UI mode is not interpretable as adaptation.\n\n")
  }
} else {
...
<!-- [CORE - PRIMARY] RQ1/RQ3: Complementary performance metric to throughput -->
...
**Sample Size:** `r get_n_string(df %>% filter(modality == "hand"))` participants for hand modality (mouse users only), `r get_n_string(df %>% filter(modality == "gaze"))` participants for gaze modality (mouse + trackpad users).
...
**Note on unbalanced design:** Same as throughput analysis: hand modality N=`r {stats_tp <- compute_input_device_stats(df_iso); stats_tp$hand_n}` (mouse users only), gaze modality N=`r stats_tp$gaze_n` (`r stats_tp$mouse_n` mouse + `r stats_tp$trackpad_n` trackpad users). Type III ANOVA with sum-to-zero contrasts handles this appropriately (Fox & Weisberg, 2019).
...
if (exists("model_rt") && !is.null(model_rt)) {
  ...
  if (nrow(ui_mode_row) > 0 && !is.na(ui_mode_row$`F value`[1])) {
    ...
    cat("**UI Mode Effect (Omnibus):** The main effect of UI mode on movement time was ",
        ifelse(p_ui < .05, "significant", "non-significant"),
        ", ", format_F_apa(F_ui, df1_ui, df2_ui, p_ui),
        ", η²p = ", sprintf("%.3f", eta_ui),
        " (", interpret_eta_sq(eta_ui), " effect). ", sep = "")
    cat("**Note:** This omnibus UI mode effect is diluted by the fact that HAND width inflation did not execute, so UI mode is not interpretable as an adaptive manipulation for hand. The interpretable test of adaptation comes from the **gaze-only UI Mode × Pressure follow-up model** below.\n\n")
  }
  ...
  # Gaze-only follow-up results
  if (exists("model_rt_gaze") && !is.null(model_rt_gaze) && exists("emm_rt_gaze") && !is.null(emm_rt_gaze)) {
    cat("##### Gaze-Only Model: UI Mode × Pressure (Primary Adaptive Test)\n\n")
    
    # ANOVA for gaze-only model
    anova_rt_gaze <- anova(model_rt_gaze, type = "III")
    print(anova_rt_gaze)
    cat("\n")
    
    # EMMs for gaze-only
    emm_rt_gaze_df <- as.data.frame(emm_rt_gaze)
    ci_lower <- if ("lower.CL" %in% names(emm_rt_gaze_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_rt_gaze_df)) "asymp.LCL" else NULL
    ci_upper <- if ("upper.CL" %in% names(emm_rt_gaze_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_rt_gaze_df)) "asymp.UCL" else NULL
    
    cat("**Estimated Marginal Means (Gaze-only):**\n")
    emm_rt_gaze_table <- emm_rt_gaze_df %>%
      mutate(
        `UI Mode` = str_to_title(ui_mode),
        Pressure = as.character(pressure),
        `Mean RT (s)` = round(emmean, 3),
        `95% CI Lower` = if (!is.null(ci_lower)) round(.data[[ci_lower]], 3) else NA_real_,
        `95% CI Upper` = if (!is.null(ci_upper)) round(.data[[ci_upper]], 3) else NA_real_
      ) %>%
      select(`UI Mode`, Pressure, `Mean RT (s)`, `95% CI Lower`, `95% CI Upper`)
    print(knitr::kable(emm_rt_gaze_table, caption = "Estimated Marginal Means for Movement Time: Gaze-only"))
    cat("\n")
    
    # Key contrasts
    if (exists("pairs_rt_gaze") && !is.null(pairs_rt_gaze)) {
      cat("**Key Contrasts (Gaze-only, Holm-adjusted):**\n")
      pairs_rt_gaze_df <- as.data.frame(pairs_rt_gaze)
      # Focus on adaptive vs static under pressure ON and OFF
      key_contrasts <- pairs_rt_gaze_df %>%
        filter(grepl("adaptive.*static|static.*adaptive", contrast, ignore.case = TRUE))
      if (nrow(key_contrasts) > 0) {
        print(knitr::kable(key_contrasts %>% head(6), digits = 3))
      }
  ...
  # Hand-only follow-up results (pressure effect only)
  if (exists("model_rt_hand") && !is.null(model_rt_hand) && exists("emm_rt_hand") && !is.null(emm_rt_hand)) {
    cat("#### Hand-Only Follow-up: Pressure Effect\n\n")
    cat("*Note:* UI mode is excluded from hand models by design because width scaling did not execute.\n\n")
    
    # ANOVA for hand-only model
    anova_rt_hand <- anova(model_rt_hand, type = "III")
    print(anova_rt_hand)
    cat("\n")
    
    # EMMs for hand-only
    emm_rt_hand_df <- as.data.frame(emm_rt_hand)
    ci_lower <- if ("lower.CL" %in% names(emm_rt_hand_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_rt_hand_df)) "asymp.LCL" else NULL
    ci_upper <- if ("upper.CL" %in% names(emm_rt_hand_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_rt_hand_df)) "asymp.UCL" else NULL
    
    cat("**Estimated Marginal Means (Hand-only):**\n")
    emm_rt_hand_table <- emm_rt_hand_df %>%
      mutate(
        Pressure = as.character(pressure),
        `Mean RT (s)` = round(emmean, 3),
        `95% CI Lower` = if (!is.null(ci_lower)) round(.data[[ci_lower]], 3) else NA_real_,
      ...
      select(Pressure, `Mean RT (s)`, `95% CI Lower`, `95% CI Upper`)
    print(knitr::kable(emm_rt_hand_table, caption = "Estimated Marginal Means for Movement Time: Hand-only (pressure effect)"))
    
    if (exists("pairs_rt_hand") && !is.null(pairs_rt_hand)) {
      cat("\n**Pressure Contrast (Hand-only, Holm-adjusted):**\n")
      pairs_rt_hand_df <- as.data.frame(pairs_rt_hand)
      print(knitr::kable(pairs_rt_hand_df, digits = 3))
    }
    cat("\n")
  }
  ...
  # Structured interpretation summary
  cat("\n#### Interpretation Summary\n\n")
  cat("**Omnibus takeaway:** The 2×2×2 factorial model shows a large modality main effect (hand < gaze RT) dominates movement time differences, with UI mode effects diluted by hand non-manipulation.\n\n")
  
  if (exists("model_rt_gaze") && !is.null(model_rt_gaze)) {
    cat("**Gaze-only takeaway:** The gaze-only UI Mode × Pressure model is the primary adaptation effectiveness test. Declutter effects on movement time in gaze are evaluable here.\n\n")
  }
  
  if (exists("model_rt_hand") && !is.null(model_rt_hand)) {
    cat("**Hand-only takeaway:** Hand models exclude UI mode by design because width_scale_factor never deviated from 1.0 (no manipulation occurred). Pressure effects are evaluable in hand, but UI mode is not interpretable as adaptation.\n\n")
  }
} else {
...
<!-- [CORE - VALIDATION] H4: Validation that task works as expected (manipulation check) -->
...
**Sample Size:** `r get_n_string(df_iso %>% filter(modality == "hand"))` participants for hand modality (mouse users only), `r get_n_string(df_iso %>% filter(modality == "gaze"))` participants for gaze modality (mouse + trackpad users).
...
<!-- [CORE - PRIMARY] RQ1/RQ3: Error analysis, key finding (gaze errors are slips) -->
...
**Sample Size:** `r get_n_string(df_all_trials %>% filter(modality == "hand"))` participants for hand modality (mouse users only), `r get_n_string(df_all_trials %>% filter(modality == "gaze"))` participants for gaze modality (mouse + trackpad users).
...
if (exists("model_err") && !is.null(model_err)) {
  ...
  if (requireNamespace("car", quietly = TRUE)) {
    library(car)
    cat("#### ANOVA Table (Type III for unbalanced design)\n")
    print(Anova(model_err, type = "III"))
  } else {
    cat("**Note:** 'car' package not available. Using lmerTest::anova() instead.\n")
    print(anova(model_err, type = "III"))
  }
  ...
  # Pairwise comparisons (omnibus)
  if (exists("pairs_err") && !is.null(pairs_err)) {
    cat("\n#### Pairwise Comparisons (Omnibus, Holm-adjusted)\n")
    cat("*Note:* Omnibus UI mode effects are diluted by hand non-manipulation. See gaze-only follow-up below.\n\n")
  ...
  # Gaze-only follow-up results
  if (exists("model_err_gaze") && !is.null(model_err_gaze) && exists("emm_err_gaze") && !is.null(emm_err_gaze)) {
    cat("##### Gaze-Only Model: UI Mode × Pressure (Primary Adaptive Test)\n\n")
    
    # ANOVA for gaze-only model
    if (requireNamespace("car", quietly = TRUE)) {
      cat("**ANOVA (Type III):**\n")
      print(Anova(model_err_gaze, type = "III"))
    } else {
      print(anova(model_err_gaze, type = "III"))
    }
    cat("\n")
    ...
    # EMMs for gaze-only
    emm_err_gaze_df <- as.data.frame(emm_err_gaze)
    ci_lower <- if ("lower.CL" %in% names(emm_err_gaze_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_err_gaze_df)) "asymp.LCL" else NULL
    ci_upper <- if ("upper.CL" %in% names(emm_err_gaze_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_err_gaze_df)) "asymp.UCL" else NULL
    
    cat("**Estimated Marginal Means (Gaze-only, response scale):**\n")
    emm_err_gaze_table <- emm_err_gaze_df %>%
      mutate(
        `UI Mode` = str_to_title(ui_mode),
        Pressure = as.character(pressure),
        `Mean Error Rate` = round(100 * prob, 1),
        `95% CI Lower` = if (!is.null(ci_lower)) round(100 * .data[[ci_lower]], 1) else NA_real_,
        `95% CI Upper` = if (!is.null(ci_upper)) round(100 * .data[[ci_upper]], 1) else NA_real_
      ) %>%
      select(`UI Mode`, Pressure, `Mean Error Rate`, `95% CI Lower`, `95% CI Upper`)
    print(knitr::kable(emm_err_gaze_table, caption = "Estimated Marginal Means for Error Rate: Gaze-only (UI Mode × Pressure)"))
    cat("\n")
    
    # Key contrasts
    if (exists("pairs_err_gaze") && !is.null(pairs_err_gaze)) {
      cat("**Key Contrasts (Gaze-only, Holm-adjusted, odds ratio scale):**\n")
      pairs_err_gaze_df <- as.data.frame(pairs_err_gaze)
      # Focus on adaptive vs static under pressure ON and OFF
      key_contrasts <- pairs_err_gaze_df %>%
        filter(grepl("adaptive.*static|static.*adaptive", contrast, ignore.case = TRUE))
      if (nrow(key_contrasts) > 0) {
        print(knitr::kable(key_contrasts %>% head(6), digits = 3))
      }
  ...
  # Hand-only follow-up results (pressure effect only, if variance sufficient)
  if (exists("model_err_hand") && !is.null(model_err_hand) && exists("emm_err_hand") && !is.null(emm_err_hand)) {
    cat("#### Hand-Only Follow-up: Pressure Effect\n\n")
    cat("*Note:* UI mode is excluded from hand models by design because width scaling did not execute.\n\n")
    ...
    if (requireNamespace("car", quietly = TRUE)) {
      cat("**ANOVA (Type III):**\n")
      print(Anova(model_err_hand, type = "III"))
    } else {
      print(anova(model_err_hand, type = "III"))
    }
    cat("\n")
    ...
    # EMMs for hand-only
    emm_err_hand_df <- as.data.frame(emm_err_hand)
    ci_lower <- if ("lower.CL" %in% names(emm_err_hand_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_err_hand_df)) "asymp.LCL" else NULL
    ci_upper <- if ("upper.CL" %in% names(emm_err_hand_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_err_hand_df)) "asymp.UCL" else NULL
    
    cat("**Estimated Marginal Means (Hand-only, response scale):**\n")
    emm_err_hand_table <- emm_err_hand_df %>%
      mutate(
        Pressure = as.character(pressure),
        `Mean Error Rate` = round(100 * prob, 1),
        `95% CI Lower` = if (!is.null(ci_lower)) round(100 * .data[[ci_lower]], 1) else NA_real_,
      ...
      select(Pressure, `Mean Error Rate`, `95% CI Lower`, `95% CI Upper`)
    print(knitr::kable(emm_err_hand_table, caption = "Estimated Marginal Means for Error Rate: Hand-only (pressure effect)"))
    
    if (exists("pairs_err_hand") && !is.null(pairs_err_hand)) {
      cat("\n**Pressure Contrast (Hand-only, Holm-adjusted, odds ratio scale):**\n")
      pairs_err_hand_df <- as.data.frame(pairs_err_hand)
      print(knitr::kable(pairs_err_hand_df, digits = 3))
    }
    cat("\n")
  }
  ...
  # Structured interpretation summary
  cat("\n#### Interpretation Summary\n\n")
  cat("**Omnibus takeaway:** The 2×2×2 factorial model shows a large modality main effect (gaze > hand error rates) dominates error rate differences, with UI mode effects diluted by hand non-manipulation.\n\n")
  
  if (exists("model_err_gaze") && !is.null(model_err_gaze)) {
    cat("**Gaze-only takeaway:** The gaze-only UI Mode × Pressure model is the primary adaptation effectiveness test. Declutter effects on error rates in gaze are evaluable here.\n\n")
  }
  
  if (exists("model_err_hand") && !is.null(model_err_hand)) {
    cat("**Hand-only takeaway:** Hand models exclude UI mode by design because width_scale_factor never deviated from 1.0 (no manipulation occurred). Pressure effects are evaluable in hand, but UI mode is not interpretable as adaptation.\n\n")
  }
} else {
...
<!-- [CORE - VALIDATION] H4: Effective Width (We) is core validation metric (ISO 9241-9). -->
<!-- [SUPPLEMENT] Scatter plots and Midas Touch re-entries below are exploratory. -->
...
```{r we-statistical-tests}
# Statistical tests for Effective Width
if (exists("df_iso") && nrow(df_iso) > 0 && "We" %in% names(df_iso)) {
  df_we_stats <- df_iso %>%
    filter(!is.na(We), is.finite(We)) %>%
    mutate(
      modality = factor(modality, levels = c("hand", "gaze")),
      ui_mode = factor(ui_mode, levels = c("static", "adaptive")),
      pressure = factor(pressure),
  ...
  if (nrow(df_we_stats) > 0 && n_distinct(df_we_stats$pid) >= 2) {
    # Check if IDe_bits is available
    has_IDe <- "IDe_bits" %in% names(df_we_stats) && !all(is.na(df_we_stats$IDe_bits))
    
    # Fit mixed-effects model
    tryCatch({
      if (has_IDe) {
        m_we <- lmer(We ~ modality * ui_mode * pressure + IDe_bits + (1 | pid), 
                    data = df_we_stats)
      } else {
        m_we <- lmer(We ~ modality * ui_mode * pressure + (1 | pid), 
                    data = df_we_stats)
      }
      
      anova_we <- anova(m_we, type = "III")
      
      cat("### ANOVA: Effective Width (Type III)\n\n")
      print(kable(anova_we, digits = 4, 
                  caption = paste0("Mixed-effects model: We ~ Modality × UI Mode × Pressure (", 
                                 get_n_string(df_we_stats), " participants)")))
      cat("\n")
      
      # Extract and format main effects in APA style
      if ("modality" %in% rownames(anova_we)) {
        F_mod <- anova_we["modality", "F"]
        df1_mod <- anova_we["modality", "NumDF"]
        df2_mod <- anova_we["modality", "DenDF"]
        p_mod <- anova_we["modality", "Pr(>F)"]
        cat("**Modality effect:** ", format_F_apa(F_mod, df1_mod, df2_mod, p_mod), "\n\n")
      }
      
      if ("ui_mode" %in% rownames(anova_we)) {
        F_ui <- anova_we["ui_mode", "F"]
        df1_ui <- anova_we["ui_mode", "NumDF"]
        df2_ui <- anova_we["ui_mode", "DenDF"]
        p_ui <- anova_we["ui_mode", "Pr(>F)"]
        cat("**UI mode effect:** ", format_F_apa(F_ui, df1_ui, df2_ui, p_ui), "\n\n")
      }
      
      if ("modality:ui_mode" %in% rownames(anova_we)) {
        F_int <- anova_we["modality:ui_mode", "F"]
        df1_int <- anova_we["modality:ui_mode", "NumDF"]
        df2_int <- anova_we["modality:ui_mode", "DenDF"]
        p_int <- anova_we["modality:ui_mode", "Pr(>F)"]
        cat("**Modality × UI mode interaction:** ", format_F_apa(F_int, df1_int, df2_int, p_int), "\n\n")
      }
      
      # Estimated marginal means (include pressure)
      emm_we <- emmeans(m_we, ~ ui_mode | modality * pressure)
      emm_we_summary <- summary(emm_we)
      
      cat("### Estimated Marginal Means (by Modality × Pressure)\n\n")
      print(kable(emm_we_summary, digits = 2, 
                  caption = "Effective Width (px) by Modality, UI Mode, and Pressure"))
      cat("\n")
      
      # Follow-up: Gaze-only and Hand-only models
      df_we_gaze <- df_we_stats %>% filter(modality == "gaze")
      df_we_hand <- df_we_stats %>% filter(modality == "hand")
      
      # Gaze-only We model
      m_we_gaze <- NULL
      emm_we_gaze <- NULL
      pairs_we_gaze <- NULL
      if (nrow(df_we_gaze) > 0 && n_distinct(df_we_gaze$pid) >= 2) {
        if (has_IDe) {
          m_we_gaze <- tryCatch({
            lmer(We ~ ui_mode * pressure + IDe_bits + (1 | pid), data = df_we_gaze)
          }, error = function(e) NULL)
        } else {
          m_we_gaze <- tryCatch({
            lmer(We ~ ui_mode * pressure + (1 | pid), data = df_we_gaze)
          }, error = function(e) NULL)
        }
        
        if (!is.null(m_we_gaze)) {
          emm_we_gaze <- tryCatch({
            emmeans(m_we_gaze, ~ ui_mode * pressure)
          }, error = function(e) NULL)
          
          if (!is.null(emm_we_gaze)) {
            pairs_we_gaze <- pairs(emm_we_gaze, adjust = "holm")
          }
        }
      }
      
      # Hand-only We model (pressure only)
      m_we_hand <- NULL
      emm_we_hand <- NULL
      pairs_we_hand <- NULL
      if (nrow(df_we_hand) > 0 && n_distinct(df_we_hand$pid) >= 2) {
        if (has_IDe) {
          m_we_hand <- tryCatch({
            lmer(We ~ pressure + IDe_bits + (1 | pid), data = df_we_hand)
          }, error = function(e) NULL)
        } else {
          m_we_hand <- tryCatch({
            lmer(We ~ pressure + (1 | pid), data = df_we_hand)
          }, error = function(e) NULL)
        }
        
        if (!is.null(m_we_hand)) {
          emm_we_hand <- tryCatch({
            emmeans(m_we_hand, ~ pressure)
          }, error = function(e) NULL)
          
          if (!is.null(emm_we_hand)) {
            pairs_we_hand <- pairs(emm_we_hand, adjust = "holm")
          }
        }
      }
      
      # Pairwise comparisons (omnibus)
      pairs_we <- pairs(emm_we, adjust = "holm")
      pairs_we_df <- as.data.frame(pairs_we)
      
      if (nrow(pairs_we_df) > 0) {
        pairs_we_apa <- pairs_we_df %>%
          mutate(
            `p-value` = format_p_apa(p.value),
            `t-ratio` = round(t.ratio, 2)
          ) %>%
          select(contrast, estimate, SE, `t-ratio`, `p-value`, df)
        
        cat("### Pairwise Comparisons (Holm-adjusted)\n\n")
        print(kable(pairs_we_apa, digits = 3, 
                    caption = "UI Mode comparisons within each Modality"))
        cat("\n")
        
        # APA-formatted text summary
        cat("**APA-formatted summary (omnibus):** ")
        sig_pairs <- pairs_we_df %>% filter(p.value < 0.05)
        if (nrow(sig_pairs) > 0) {
          for (i in 1:min(3, nrow(sig_pairs))) {
            mod <- sig_pairs$modality[i]
            est <- round(sig_pairs$estimate[i], 2)
            t_val <- round(sig_pairs$t.ratio[i], 2)
            df_val <- round(sig_pairs$df[i], 1)
            p_val <- sig_pairs$p.value[i]
            cat(sprintf("In %s modality, adaptive UI %s effective width by %.2f px, %s. ", 
                       str_to_title(mod),
                       ifelse(est < 0, "reduced", "increased"),
                       abs(est),
                       format_t_apa(t_val, df_val, p_val)))
          }
        } else {
          cat("No significant differences in effective width between UI modes within modalities (all p > 0.05).")
        }
        cat("\n\n")
        
        # Gaze-only follow-up
        if (!is.null(emm_we_gaze) && !is.null(pairs_we_gaze)) {
          cat("#### Gaze-Only Follow-up: UI Mode × Pressure\n\n")
          cat("**Estimated Marginal Means (Gaze-only):**\n")
          emm_we_gaze_df <- as.data.frame(emm_we_gaze)
          emm_we_gaze_table <- emm_we_gaze_df %>%
            mutate(
              `UI Mode` = str_to_title(ui_mode),
              Pressure = as.character(pressure),
              `Mean We (px)` = round(emmean, 2)
            ) %>%
            select(`UI Mode`, Pressure, `Mean We (px)`)
          print(kable(emm_we_gaze_table, digits = 2, 
                      caption = "Effective Width: Gaze-only (UI Mode × Pressure)"))
          
          pairs_we_gaze_df <- as.data.frame(pairs_we_gaze)
          key_contrasts <- pairs_we_gaze_df %>%
            filter(grepl("adaptive.*static|static.*adaptive", contrast, ignore.case = TRUE))
          if (nrow(key_contrasts) > 0) {
            cat("\n**Key Contrasts (Gaze-only, Holm-adjusted):**\n")
            print(kable(key_contrasts %>% head(6), digits = 3))
          }
          cat("\n")
        }
        
        # Hand-only follow-up
        if (!is.null(emm_we_hand) && !is.null(pairs_we_hand)) {
          cat("#### Hand-Only Follow-up: Pressure Effect\n\n")
          cat("*Note:* UI mode is excluded from hand models by design because width scaling did not execute.\n\n")
          cat("**Estimated Marginal Means (Hand-only):**\n")
          emm_we_hand_df <- as.data.frame(emm_we_hand)
          emm_we_hand_table <- emm_we_hand_df %>%
            mutate(
              Pressure = as.character(pressure),
              `Mean We (px)` = round(emmean, 2)
            ) %>%
            select(Pressure, `Mean We (px)`)
          print(kable(emm_we_hand_table, digits = 2, 
                      caption = "Effective Width: Hand-only (pressure effect)"))
          
          pairs_we_hand_df <- as.data.frame(pairs_we_hand)
          cat("\n**Pressure Contrast (Hand-only, Holm-adjusted):**\n")
          print(kable(pairs_we_hand_df, digits = 3))
          cat("\n")
        }
        
        # Structured interpretation summary
        cat("#### Interpretation Summary\n\n")
        cat("**Omnibus takeaway:** The 2×2×2 factorial model shows a modality main effect (gaze > hand We) dominates effective width differences, with UI mode effects diluted by hand non-manipulation.\n\n")
        
        if (!is.null(emm_we_gaze) && !is.null(pairs_we_gaze)) {
          cat("**Gaze-only takeaway:** The gaze-only UI Mode × Pressure model is the primary adaptation effectiveness test. Declutter effects on spatial precision (We) in gaze are evaluable here.\n\n")
        }
        
        if (!is.null(emm_we_hand) && !is.null(pairs_we_hand)) {
          cat("**Hand-only takeaway:** Hand models exclude UI mode by design because width_scale_factor never deviated from 1.0 (no manipulation occurred). Pressure effects on We are evaluable in hand, but UI mode is not interpretable as adaptation.\n\n")
        }
      }
    }, error = function(e) {
      cat("⚠ Could not fit Effective Width model:", conditionMessage(e), "\n\n")
    })
  } else {
    cat("⚠ Insufficient data for Effective Width statistical tests.\n\n")
  }
} else {
  cat("⚠ Effective Width data not available for statistical tests.\n\n")
}
```

<!-- [SUPPLEMENT - EXPLORATORY] Endpoint accuracy scatter plots: Visualization, not core finding -->
...
<!-- [SUPPLEMENT - EXPLORATORY] Midas Touch re-entries: Interesting descriptive finding, not pre-registered -->
...
<!-- [CORE - PRIMARY] RQ2: Workload assessment, pre-registered hypothesis -->
...
**Sample Size:** `r get_n_string(df_raw %>% filter(!is.na(pid), !is.na(tlx_mental), modality == "hand"))` participants for hand modality (mouse users only), `r get_n_string(df_raw %>% filter(!is.na(pid), !is.na(tlx_mental), modality == "gaze"))` participants for gaze modality (mouse + trackpad users).
...
**Note on unbalanced design:** Same as other analyses: hand modality N=`r {stats_tlx <- compute_input_device_stats(df_raw %>% filter(!is.na(tlx_mental))); stats_tlx$hand_n}` (mouse users only), gaze modality N=`r stats_tlx$gaze_n` (`r stats_tlx$mouse_n` mouse + `r stats_tlx$trackpad_n` trackpad users). Type III ANOVA with sum-to-zero contrasts handles this appropriately (Fox & Weisberg, 2019).
...
if(all(tlx_cols %in% names(df_raw))) {
  ...
  if (nrow(df_tlx_overall) > 0 && n_distinct(df_tlx_overall$pid) > 1) {
    ...
    if (!is.null(model_tlx)) {
      ...
      # Compute emm_tlx once right after model fits
      emm_tlx <- tryCatch({
      ...
      if (!is.null(emm_tlx)) {
        pairs_tlx <- tryCatch({
          pairs(emm_tlx, adjust = "holm")
        }, error = function(e) NULL)
      }
      
      # Gaze-only TLX model
      df_tlx_gaze <- df_tlx_overall %>% filter(modality == "gaze")
      model_tlx_gaze <- NULL
      emm_tlx_gaze <- NULL
      pairs_tlx_gaze <- NULL
      if (nrow(df_tlx_gaze) > 0 && n_distinct(df_tlx_gaze$pid) >= 2) {
        model_tlx_gaze <- tryCatch({
          lmer(overall_tlx ~ ui_mode + (1|pid), data = df_tlx_gaze, 
               REML = FALSE, control = lmerControl(optimizer = "bobyqa"))
        }, error = function(e) NULL)
        
        if (!is.null(model_tlx_gaze)) {
          emm_tlx_gaze <- tryCatch({
            emmeans(model_tlx_gaze, ~ ui_mode)
          }, error = function(e) NULL)
          
          if (!is.null(emm_tlx_gaze)) {
            pairs_tlx_gaze <- tryCatch({
              pairs(emm_tlx_gaze, adjust = "holm")
            }, error = function(e) NULL)
          }
        }
      }
      ...
      if (nrow(ui_mode_row) > 0 && !is.na(ui_mode_row$`F value`[1])) {
        ...
        cat("**UI Mode Effect (Omnibus):** The main effect of UI mode on workload was ",
            ifelse(p_ui < .05, "significant", "non-significant"),
            ", ", format_F_apa(F_ui, df1_ui, df2_ui, p_ui),
            ", η²p = ", sprintf("%.3f", eta_ui),
            " (", interpret_eta_sq(eta_ui), " effect). ", sep = "")
        cat("**Note:** This omnibus UI mode effect is diluted by the fact that HAND width inflation did not execute, so UI mode is not interpretable as an adaptive manipulation for hand. The interpretable test of adaptation comes from the **gaze-only UI Mode follow-up model** below.\n\n")
      }
      
      # Gaze-only follow-up results
      if (!is.null(model_tlx_gaze) && !is.null(emm_tlx_gaze)) {
        cat("#### Gaze-Only Follow-up: UI Mode (Primary Adaptive Test)\n\n")
        
        # ANOVA for gaze-only model
        anova_tlx_gaze <- anova(model_tlx_gaze, type = "III")
        print(anova_tlx_gaze)
        cat("\n")
        
        # EMMs for gaze-only
        emm_tlx_gaze_df <- as.data.frame(emm_tlx_gaze)
        ci_lower <- if ("lower.CL" %in% names(emm_tlx_gaze_df)) "lower.CL" else if ("asymp.LCL" %in% names(emm_tlx_gaze_df)) "asymp.LCL" else NULL
        ci_upper <- if ("upper.CL" %in% names(emm_tlx_gaze_df)) "upper.CL" else if ("asymp.UCL" %in% names(emm_tlx_gaze_df)) "asymp.UCL" else NULL
        
        cat("**Estimated Marginal Means (Gaze-only):**\n")
        emm_tlx_gaze_table <- emm_tlx_gaze_df %>%
          mutate(
            `UI Mode` = str_to_title(ui_mode),
          ...
          select(`UI Mode`, `Mean TLX`, `95% CI Lower`, `95% CI Upper`)
        print(knitr::kable(emm_tlx_gaze_table, caption = "Estimated Marginal Means for Overall TLX: Gaze-only"))
        
        if (!is.null(pairs_tlx_gaze)) {
          cat("\n**Contrast (Gaze-only, Holm-adjusted):**\n")
          pairs_tlx_gaze_df <- as.data.frame(pairs_tlx_gaze)
          print(knitr::kable(pairs_tlx_gaze_df, digits = 3))
        }
        cat("\n")
      }
      ...
      # EMMs with 95% CIs (already computed above)
      if (!is.null(emm_tlx)) {
      ...
      # Structured interpretation summary
      cat("\n#### Interpretation Summary\n\n")
      cat("**Omnibus takeaway:** The 2×2 factorial model (modality × UI mode) shows a modality main effect (gaze > hand workload) dominates TLX differences, with UI mode effects diluted by hand non-manipulation.\n\n")
      
      if (!is.null(model_tlx_gaze) && !is.null(emm_tlx_gaze)) {
        cat("**Gaze-only takeaway:** The gaze-only UI Mode model is the primary adaptation effectiveness test. Declutter effects on workload in gaze are evaluable here (TLX is block-level and typically does not vary by pressure).\n\n")
      }
      
      cat("**Hand-only note:** Hand UI mode is not interpretable as adaptation because width_scale_factor never deviated from 1.0 (no manipulation occurred). TLX is block-level and typically does not vary by pressure, so pressure-only hand models are not reported for TLX.\n\n")
    }
  }
}
```
...
<!-- [SUPPLEMENT] Qualitative/Supplementary: Participant awareness, not core RQ -->
...
<!-- [SUPPLEMENT - EXPLORATORY] Practice effects over time, not primary outcome -->
...
<!-- [SUPPLEMENT - ADVANCED/EXPLORATORY] Submovements, verification time: Advanced movement analysis -->
...
<!-- [SUPPLEMENT - DESCRIPTIVE] Error type breakdown (miss, timeout, slip): Descriptive analysis -->
...
<!-- [SUPPLEMENT - EXPLORATORY] Temporal patterns, not pre-registered -->
...
<!-- [SUPPLEMENT - VISUALIZATION/EXPLORATORY] Spatial distribution of endpoints: Descriptive visualization -->
...
**Note:** This section includes both descriptive visualizations and inferential statistical tests. At N=`r n_distinct(df_all_trials %>% filter(!is.na(target_center_x), !is.na(target_center_y)) %>% pull(pid))`, spatial analyses provide insights into XR-specific patterns (e.g., top vs bottom of visual field) and condition differences.
...
if ("target_center_x" %in% names(df_all_trials) && "target_center_y" %in% names(df_all_trials)) {
  ...
  if (nrow(df_spatial) > 0) {
    ...
    # Statistical tests for spatial position effects
    cat("\n### Statistical Tests: Spatial Position Effects\n\n")
    
    # Prepare data for statistical tests
    df_spatial_tests <- df_all_trials %>%
      filter(!is.na(target_center_x), !is.na(target_center_y)) %>%
      mutate(
        # Create screen regions: center vs periphery
        screen_region = case_when(
          target_center_x < quantile(target_center_x, 0.33, na.rm = TRUE) ~ "Left",
          target_center_x > quantile(target_center_x, 0.67, na.rm = TRUE) ~ "Right",
          TRUE ~ "Center"
        ),
        vertical_region = case_when(
          target_center_y < quantile(target_center_y, 0.33, na.rm = TRUE) ~ "Top",
          target_center_y > quantile(target_center_y, 0.67, na.rm = TRUE) ~ "Bottom",
          TRUE ~ "Middle"
        ),
        screen_region = factor(screen_region, levels = c("Left", "Center", "Right")),
        vertical_region = factor(vertical_region, levels = c("Top", "Middle", "Bottom"))
      )
    
    if (nrow(df_spatial_tests) > 0 && n_distinct(df_spatial_tests$screen_region) >= 2) {
      # Test 1: Does performance vary by screen region (horizontal)?
      tryCatch({
        df_spatial_rt_data <- df_spatial_tests %>% 
          filter(is_correct == TRUE | is_correct == 1)
        # Include IDe_bits if available
        if ("IDe_bits" %in% names(df_spatial_rt_data) && !all(is.na(df_spatial_rt_data$IDe_bits))) {
          m_spatial_rt <- lmer(log(rt_ms) ~ screen_region * modality * ui_mode + IDe_bits + 
                                (1 | pid), data = df_spatial_rt_data)
        } else {
          m_spatial_rt <- lmer(log(rt_ms) ~ screen_region * modality * ui_mode + 
                                (1 | pid), data = df_spatial_rt_data)
        }
        anova_spatial_rt <- anova(m_spatial_rt)
        
        cat("**Movement Time by Screen Region (Horizontal):**\n")
        print(kable(anova_spatial_rt, digits = 4, caption = "ANOVA: RT ~ Screen Region × Modality × UI Mode"))
        cat("\n")
        
        # APA-formatted summary
        if ("screen_region" %in% rownames(anova_spatial_rt)) {
          F_reg <- anova_spatial_rt["screen_region", "F"]
          df1_reg <- anova_spatial_rt["screen_region", "NumDF"]
          df2_reg <- anova_spatial_rt["screen_region", "DenDF"]
          p_reg <- anova_spatial_rt["screen_region", "Pr(>F)"]
          cat("**Screen region effect:** ", format_F_apa(F_reg, df1_reg, df2_reg, p_reg), "\n\n")
        }
        
        # Post-hoc comparisons if significant
        if (any(anova_spatial_rt$`Pr(>F)` < 0.05, na.rm = TRUE)) {
          emm_spatial <- emmeans(m_spatial_rt, ~ screen_region | modality * ui_mode)
          pairs_spatial <- pairs(emm_spatial, adjust = "holm")
          pairs_spatial_df <- as.data.frame(pairs_spatial)
          
          pairs_spatial_apa <- pairs_spatial_df %>%
            mutate(`p-value` = format_p_apa(p.value)) %>%
            select(contrast, modality, ui_mode, estimate, SE, `p-value`, df)
          
          cat("**Pairwise Comparisons (Holm-adjusted):**\n")
          print(kable(pairs_spatial_apa, digits = 4))
          cat("\n")
        }
      }, error = function(e) {
        cat("⚠ Could not fit spatial RT model:", conditionMessage(e), "\n\n")
      })
      
      # Test 2: Does error rate vary by screen region?
      tryCatch({
        df_spatial_errors_test <- df_spatial_tests %>%
          filter(!is.na(is_correct))
        
        if (nrow(df_spatial_errors_test) > 0 && 
            mean(df_spatial_errors_test$is_correct, na.rm = TRUE) > 0.01 &&
            mean(df_spatial_errors_test$is_correct, na.rm = TRUE) < 0.99) {
          # Include IDe_bits if available
          if ("IDe_bits" %in% names(df_spatial_errors_test) && !all(is.na(df_spatial_errors_test$IDe_bits))) {
            m_spatial_err <- glmer(is_correct ~ screen_region * modality * ui_mode + IDe_bits + 
                                    (1 | pid),
                                  data = df_spatial_errors_test, family = binomial)
          } else {
            m_spatial_err <- glmer(is_correct ~ screen_region * modality * ui_mode + 
                                    (1 | pid),
                                  data = df_spatial_errors_test, family = binomial)
          }
          anova_spatial_err <- anova(m_spatial_err)
          
          cat("**Error Rate by Screen Region (Horizontal):**\n")
          print(kable(anova_spatial_err, digits = 4, caption = "ANOVA: Error Rate ~ Screen Region × Modality × UI Mode"))
          cat("\n")
          
          # APA-formatted summary
          if ("screen_region" %in% rownames(anova_spatial_err)) {
            F_reg <- anova_spatial_err["screen_region", "F"]
            df1_reg <- anova_spatial_err["screen_region", "NumDF"]
            df2_reg <- anova_spatial_err["screen_region", "DenDF"]
            p_reg <- anova_spatial_err["screen_region", "Pr(>F)"]
            cat("**Screen region effect:** ", format_chisq_apa(F_reg, df1_reg, p_reg), "\n\n")
          }
        }
      }, error = function(e) {
        cat("⚠ Could not fit spatial error model:", conditionMessage(e), "\n\n")
      })
      
      # Test 3: Vertical region effects
      tryCatch({
        df_spatial_rt_v_data <- df_spatial_tests %>% 
          filter(is_correct == TRUE | is_correct == 1)
        # Include IDe_bits if available
        if ("IDe_bits" %in% names(df_spatial_rt_v_data) && !all(is.na(df_spatial_rt_v_data$IDe_bits))) {
          m_spatial_rt_v <- lmer(log(rt_ms) ~ vertical_region * modality * ui_mode + IDe_bits + 
                                  (1 | pid), data = df_spatial_rt_v_data)
        } else {
          m_spatial_rt_v <- lmer(log(rt_ms) ~ vertical_region * modality * ui_mode + 
                                  (1 | pid), data = df_spatial_rt_v_data)
        }
        anova_spatial_rt_v <- anova(m_spatial_rt_v)
        
        cat("**Movement Time by Screen Region (Vertical):**\n")
        print(kable(anova_spatial_rt_v, digits = 4, caption = "ANOVA: RT ~ Vertical Region × Modality × UI Mode"))
        cat("\n")
        
        # APA-formatted summary
        if ("vertical_region" %in% rownames(anova_spatial_rt_v)) {
          F_reg <- anova_spatial_rt_v["vertical_region", "F"]
          df1_reg <- anova_spatial_rt_v["vertical_region", "NumDF"]
          df2_reg <- anova_spatial_rt_v["vertical_region", "DenDF"]
          p_reg <- anova_spatial_rt_v["vertical_region", "Pr(>F)"]
          cat("**Vertical region effect:** ", format_F_apa(F_reg, df1_reg, df2_reg, p_reg), "\n\n")
        }
      }, error = function(e) {
        cat("⚠ Could not fit vertical spatial RT model:", conditionMessage(e), "\n\n")
      })
    } else {
      cat("⚠ Insufficient data for spatial statistical tests.\n\n")
    }
  } else {
...
if (nrow(df_error_gaze) > 0) {
  ...
  if (nrow(df_error_gaze) > 0) {
    ...
    # Statistical tests for error density differences
    cat("\n### Statistical Tests: Error Density Differences\n\n")
    
    if (nrow(df_error_gaze) > 0 && n_distinct(df_error_gaze$ui_mode) >= 2) {
      # Test 1: Compare error magnitude (distance from target) between UI modes
      df_error_gaze <- df_error_gaze %>%
        mutate(
          error_distance = sqrt(err_x^2 + err_y^2),
          error_abs_x = abs(err_x),
          error_abs_y = abs(err_y)
        )
      
      # Filter extreme outliers for statistical tests
      df_error_test <- df_error_gaze %>%
        filter(error_distance <= quantile(error_distance, 0.99, na.rm = TRUE))
      
      if (nrow(df_error_test) > 0) {
        # Test error distance
        tryCatch({
          # Include IDe_bits if available, otherwise use simpler model
          if ("IDe_bits" %in% names(df_error_test) && !all(is.na(df_error_test$IDe_bits))) {
            m_error_dist <- lmer(log(error_distance + 1) ~ ui_mode + IDe_bits + (1 | pid),
                                data = df_error_test)
          } else {
            m_error_dist <- lmer(log(error_distance + 1) ~ ui_mode + (1 | pid),
                                data = df_error_test)
          }
          anova_error_dist <- anova(m_error_dist)
          
          cat("**Error Distance (Magnitude) Comparison:**\n")
          print(kable(anova_error_dist, digits = 4, caption = "ANOVA: Error Distance ~ UI Mode"))
          cat("\n")
          
          # Estimated marginal means
          emm_error <- emmeans(m_error_dist, ~ ui_mode, type = "response")
          cat("**Estimated Marginal Means (Error Distance, px):**\n")
          print(kable(summary(emm_error), digits = 3))
          cat("\n")
          
          # Pairwise comparison
          pairs_error <- pairs(emm_error, adjust = "none")
          pairs_error_df <- as.data.frame(pairs_error)
          
          pairs_error_apa <- pairs_error_df %>%
            mutate(
              `p-value` = format_p_apa(p.value),
              `t-ratio` = round(t.ratio, 2)
            ) %>%
          ...
          print(kable(pairs_error_apa, digits = 4))
          cat("\n")
          
          # APA-formatted text summary
          if (nrow(pairs_error_df) > 0 && pairs_error_df$p.value[1] < 0.05) {
            est <- round(pairs_error_df$estimate[1], 2)
            t_val <- round(pairs_error_df$t.ratio[1], 2)
            df_val <- round(pairs_error_df$df[1], 1)
            p_val <- pairs_error_df$p.value[1]
            cat("**APA summary:** Adaptive UI ", ifelse(est < 0, "reduced", "increased"), 
                " error distance by ", abs(est), " px, ", format_t_apa(t_val, df_val, p_val), ".\n\n")
          }
        }, error = function(e) {
          cat("⚠ Could not fit error distance model:", conditionMessage(e), "\n\n")
        })
        
        # Test 2: Compare error bias (systematic offset) in X and Y directions
        tryCatch({
          # X-direction bias
          if ("IDe_bits" %in% names(df_error_test) && !all(is.na(df_error_test$IDe_bits))) {
            m_error_x <- lmer(err_x ~ ui_mode + IDe_bits + (1 | pid), data = df_error_test)
          } else {
            m_error_x <- lmer(err_x ~ ui_mode + (1 | pid), data = df_error_test)
          }
          anova_error_x <- anova(m_error_x)
          
          cat("**Error Bias in X-Direction:**\n")
          print(kable(anova_error_x, digits = 4, caption = "ANOVA: Error X ~ UI Mode"))
          cat("\n")
          
          emm_error_x <- emmeans(m_error_x, ~ ui_mode)
          emm_error_x_summary <- summary(emm_error_x)
          cat("**Estimated Marginal Means (Error X, px):**\n")
          print(kable(emm_error_x_summary, digits = 3))
          cat("\n")
          
          # APA-formatted summary for X-bias
          if ("ui_mode" %in% rownames(anova_error_x)) {
            F_ui <- anova_error_x["ui_mode", "F"]
            df1_ui <- anova_error_x["ui_mode", "NumDF"]
            df2_ui <- anova_error_x["ui_mode", "DenDF"]
            p_ui <- anova_error_x["ui_mode", "Pr(>F)"]
            cat("**X-direction bias:** ", format_F_apa(F_ui, df1_ui, df2_ui, p_ui), "\n\n")
          }
          
          # Y-direction bias
          if ("IDe_bits" %in% names(df_error_test) && !all(is.na(df_error_test$IDe_bits))) {
            m_error_y <- lmer(err_y ~ ui_mode + IDe_bits + (1 | pid), data = df_error_test)
          } else {
            m_error_y <- lmer(err_y ~ ui_mode + (1 | pid), data = df_error_test)
          }
          anova_error_y <- anova(m_error_y)
          
          cat("**Error Bias in Y-Direction:**\n")
          print(kable(anova_error_y, digits = 4, caption = "ANOVA: Error Y ~ UI Mode"))
          cat("\n")
          
          emm_error_y <- emmeans(m_error_y, ~ ui_mode)
          emm_error_y_summary <- summary(emm_error_y)
          cat("**Estimated Marginal Means (Error Y, px):**\n")
          print(kable(emm_error_y_summary, digits = 3))
          cat("\n")
          
          # APA-formatted summary for Y-bias
          if ("ui_mode" %in% rownames(anova_error_y)) {
            F_ui <- anova_error_y["ui_mode", "F"]
            df1_ui <- anova_error_y["ui_mode", "NumDF"]
            df2_ui <- anova_error_y["ui_mode", "DenDF"]
            p_ui <- anova_error_y["ui_mode", "Pr(>F)"]
            cat("**Y-direction bias:** ", format_F_apa(F_ui, df1_ui, df2_ui, p_ui), "\n\n")
          }
        }, error = function(e) {
          cat("⚠ Could not fit error bias models:", conditionMessage(e), "\n\n")
        })
        
        # Test 3: Kolmogorov-Smirnov test for distribution differences
        tryCatch({
          static_dist <- df_error_test %>% filter(str_to_lower(ui_mode) == "static") %>% pull(error_distance)
          adaptive_dist <- df_error_test %>% filter(str_to_lower(ui_mode) == "adaptive") %>% pull(error_distance)
          
          if (length(static_dist) > 10 && length(adaptive_dist) > 10) {
            ks_test <- ks.test(static_dist, adaptive_dist)
            cat("**Kolmogorov-Smirnov Test: Error Distance Distributions**\n")
            cat(sprintf("D = %.4f, p = %.4f\n", ks_test$statistic, ks_test$p.value))
            if (ks_test$p.value < 0.05) {
              cat("✓ Distributions differ significantly (p < 0.05)\n")
            } else {
              cat("○ No significant difference in distributions\n")
            }
            cat("\n")
          }
        }, error = function(e) {
          cat("⚠ Could not perform KS test:", conditionMessage(e), "\n\n")
        })
        
        # Test 4: 2D Kolmogorov-Smirnov test for spatial error patterns
        # Note: This is more complex and may require additional packages
        cat("**Note:** 2D spatial pattern comparisons (e.g., 2D KS test) would require specialized packages.\n")
        cat("Current analysis focuses on univariate comparisons (distance, X-bias, Y-bias).\n\n")
      }
    } else {
      cat("⚠ Insufficient data for error density statistical tests.\n\n")
    }
  } else {
...
<!-- [SUPPLEMENT - EXPLORATORY] How adaptations activated (if at all): Policy engine analysis -->
...
# Root-cause diagnostic for width scaling non-activation
# Load policy replay diagnostics (generated by scripts/policy_replay_width_inflation_diagnostics.ts)
replay_summary_paths <- c("outputs/policy_replay_summary.csv", "../outputs/policy_replay_summary.csv")
replay_summary <- NULL

for (path in replay_summary_paths) {
  if (file.exists(path)) {
    replay_summary <- read_csv(path, show_col_types = FALSE)
    cat("Loaded policy replay summary from:", path, "\n\n")
    break
  }
}

# Also load per-participant and events if available
replay_by_pid_paths <- c("outputs/policy_replay_by_pid.csv", "../outputs/policy_replay_by_pid.csv")
replay_by_pid <- NULL
for (path in replay_by_pid_paths) {
  if (file.exists(path)) {
    replay_by_pid <- read_csv(path, show_col_types = FALSE)
    break
  }
}

replay_events_paths <- c("outputs/policy_replay_events.csv", "../outputs/policy_replay_events.csv")
replay_events <- NULL
for (path in replay_events_paths) {
  if (file.exists(path)) {
    replay_events <- read_csv(path, show_col_types = FALSE)
    break
  }
}

# Key diagnostic table
cat("### Key Diagnostics Summary\n\n")

if (!is.null(replay_summary) && nrow(replay_summary) > 0) {
  # Extract values from replay summary
  n_participants <- replay_summary$n_participants[1]
  n_ever_inflate_width <- replay_summary$n_ever_inflate_width[1]
  pct_ever_inflate_width <- replay_summary$pct_ever_inflate_width[1]
  n_max_streak_ge5 <- replay_summary$n_max_streak_ge5[1]
  pct_max_streak_ge5 <- replay_summary$pct_max_streak_ge5[1]
  total_inflate_width_events <- replay_summary$total_inflate_width_events[1]
  total_policy_events <- replay_summary$total_policy_events[1]
  n_pressure_not_enabled <- replay_summary$n_pressure_not_enabled[1]
  top_reason <- replay_summary$top_reason[1]
  
  # Check actual width_scale_factor in data
  df_hand_adapt_p1_check <- df_raw %>%
    filter(
      modality == "hand" | tolower(modality) == "hand",
      ui_mode == "adaptive" | tolower(ui_mode) == "adaptive",
  ...
  pct_scaled_actual <- if (nrow(df_hand_adapt_p1_check) > 0 && "width_scale_factor" %in% names(df_hand_adapt_p1_check)) {
    100 * mean(df_hand_adapt_p1_check$width_scale_factor != 1, na.rm = TRUE)
  } else {
    0
  }
  ...
  key_diagnostics <- data.frame(
    Metric = c(
      "Width scaling in data (Hand/Adaptive/Pressure=1)",
      "PolicyEngine emitted inflate_width (replay)",
      "Participants with ever inflate_width (replay)",
      "Participants with max bad streak >= 5 (replay)",
      "Total inflate_width events emitted (replay)",
      "Events with 'Pressure mode not enabled' (replay)"
    ),
    Value = c(
      sprintf("%.2f%%", pct_scaled_actual),
      sprintf("%d events", total_inflate_width_events),
      sprintf("%d (%.1f%%)", n_ever_inflate_width, pct_ever_inflate_width),
      sprintf("%d (%.1f%%)", n_max_streak_ge5, pct_max_streak_ge5),
      sprintf("%d", total_inflate_width_events),
      sprintf("%d", n_pressure_not_enabled)
    )
  )
  
  print(kable(key_diagnostics, caption = "Width Inflation Diagnostic Summary: Policy Replay vs. Observed Data"))
  cat("\n")
  
  # Root cause conclusion
  cat("### Root Cause Analysis\n\n")
  
  if (total_inflate_width_events > 0 && pct_scaled_actual == 0) {
    cat("**Evidence favors UI integration issue:** PolicyEngine emitted ", total_inflate_width_events, 
        " inflate_width actions (", n_ever_inflate_width, " participants, ", 
        sprintf("%.1f%%", pct_ever_inflate_width), "), but width_scale_factor remained 1.0 in all recorded trials. ",
        "This indicates the policy logic executed correctly (triggers met, hysteresis reached), but the actions were not applied to UI targets. ",
        "Likely causes: event bus not propagating policy:change events, TaskPane not updating widthScale state, or FittsTask not receiving/applying widthScale prop.\n\n", sep = "")
  } else if (total_inflate_width_events == 0 && pct_max_streak_ge5 < 50) {
    cat("**Evidence favors strict hysteresis threshold:** PolicyEngine did not emit inflate_width actions. ",
        "Only ", sprintf("%.1f%%", pct_max_streak_ge5), 
        " of participants reached the hysteresis threshold (5 consecutive triggered trials). ",
        "This suggests the hysteresis gate was too strict for the task duration and participant performance patterns.\n\n", sep = "")
  } else if (n_pressure_not_enabled > 0) {
    cat("**Evidence favors pressure gate:** ", n_pressure_not_enabled, 
        " policy events had reason 'Pressure mode not enabled', suggesting pressureEnabled runtime state did not match pressure condition labels.\n\n", sep = "")
  } else {
    cat("**Conclusion:** Policy replay diagnostics available. See outputs/policy_replay_*.csv for detailed analysis.\n\n")
  }
  
  # Top reasons from events
  if (!is.null(replay_events) && nrow(replay_events) > 0) {
    hand_events <- replay_events %>%
      filter(modality == "hand", ui_mode == "adaptive")
    
    if (nrow(hand_events) > 0) {
      reason_counts <- hand_events %>%
        group_by(reason) %>%
        summarise(n = n(), .groups = "drop") %>%
        arrange(desc(n))
      
      cat("**Top reasons in hand/adaptive policy events:**\n")
      print(kable(reason_counts %>% head(5), caption = "Policy event reasons (hand/adaptive only)"))
      cat("\n")
    }
  }
} else {
  cat("⚠ Policy replay diagnostics not found. Run scripts/policy_replay_width_inflation_diagnostics.ts first.\n")
  cat("Falling back to trigger reconstruction diagnostic...\n\n")
  
  # Fallback to trigger reconstruction
  df_hand_adapt_p1 <- df_raw %>%
    filter(
      modality == "hand",
      ui_mode == "adaptive",
      pressure == 1 | pressure == "1",
    ...
    arrange(pid, trial_number) %>%
    mutate(
      correct_bool = (correct == TRUE | correct == "true" | correct == 1),
      error = !correct_bool
    )
  ...
  if (nrow(df_hand_adapt_p1) > 0) {
    trigger_summary <- df_hand_adapt_p1 %>%
      ...
      ungroup() %>%
      group_by(pid) %>%
      summarise(max_consecutive_triggered = first(max_consecutive_triggered), .groups = "drop")
    
    n_streak_ge5 <- sum(trigger_summary$max_consecutive_triggered >= 5, na.rm = TRUE)
    pct_streak_ge5 <- 100 * n_streak_ge5 / nrow(trigger_summary)
    median_max_streak <- median(trigger_summary$max_consecutive_triggered, na.rm = TRUE)
    mean_max_streak <- mean(trigger_summary$max_consecutive_triggered, na.rm = TRUE)
    
    key_diagnostics <- data.frame(
      Metric = c(
        "Width scaling activation (Hand/Adaptive/Pressure=1)",
        "Participants with max trigger streak >= 5",
        "Median max consecutive triggered",
        "Mean max consecutive triggered"
      ),
      Value = c(
        "0.00%",
        sprintf("%.1f%%", pct_streak_ge5),
        sprintf("%.1f", median_max_streak),
        sprintf("%.2f", mean_max_streak)
      )
    )
    
    print(kable(key_diagnostics, caption = "Width Inflation Diagnostic (Trigger Reconstruction)"))
    cat("\n")
    
    if (pct_streak_ge5 < 10) {
      cat("**Evidence favors strict hysteresis threshold:** Only ", 
          sprintf("%.1f%%", pct_streak_ge5), 
          " of participants reached the hysteresis threshold (5 consecutive triggered trials).\n\n", sep = "")
    }
  }
...
cat("**Note:** Policy replay uses the actual PolicyEngine from app/src/lib/policy.ts, replaying trials through the same logic used at runtime. This provides definitive evidence of whether policy actions were emitted.\n\n")
```
...
**Status:** No activation observed (width_scale_factor == 1.0 throughout). Root cause analysis indicates policy actions were emitted (see replay diagnostics) but not applied to UI targets, suggesting an integration issue rather than threshold strictness.
...
<!-- [SUPPLEMENT - EXPLORATORY] Gaze-specific metrics (hover/dwell): Not core outcome -->
...
```{r hover-statistical-tests}
# Statistical tests for hover/dwell time
if (exists("df_hover") && nrow(df_hover) > 0 && n_distinct(df_hover$pid) >= 2) {
  # Check if IDe_bits is available
  has_IDe <- "IDe_bits" %in% names(df_hover) && !all(is.na(df_hover$IDe_bits))
  
  tryCatch({
    # Fit mixed-effects model (log transform for normality)
    if (has_IDe) {
      m_hover <- lmer(log(hover_ms) ~ ui_mode * pressure + IDe_bits + (1 | pid), 
                     data = df_hover)
    } else {
      m_hover <- lmer(log(hover_ms) ~ ui_mode * pressure + (1 | pid), 
                     data = df_hover)
    }
    
    anova_hover <- anova(m_hover)
    
    cat("### ANOVA: Hover/Dwell Time\n\n")
    print(kable(anova_hover, digits = 4, 
                caption = paste0("Mixed-effects model: log(Hover Time) ~ UI Mode × Pressure (", 
                               get_n_string(df_hover), " participants)")))
    cat("\n")
    
    # Extract and format main effects in APA style
    if ("ui_mode" %in% rownames(anova_hover)) {
      F_ui <- anova_hover["ui_mode", "F"]
      df1_ui <- anova_hover["ui_mode", "NumDF"]
      df2_ui <- anova_hover["ui_mode", "DenDF"]
      p_ui <- anova_hover["ui_mode", "Pr(>F)"]
      cat("**UI mode effect:** ", format_F_apa(F_ui, df1_ui, df2_ui, p_ui), "\n\n")
    }
    
    if ("pressure" %in% rownames(anova_hover)) {
      F_press <- anova_hover["pressure", "F"]
      df1_press <- anova_hover["pressure", "NumDF"]
      df2_press <- anova_hover["pressure", "DenDF"]
      p_press <- anova_hover["pressure", "Pr(>F)"]
      cat("**Pressure effect:** ", format_F_apa(F_press, df1_press, df2_press, p_press), "\n\n")
    }
    
    if ("ui_mode:pressure" %in% rownames(anova_hover)) {
      F_int <- anova_hover["ui_mode:pressure", "F"]
      df1_int <- anova_hover["ui_mode:pressure", "NumDF"]
      df2_int <- anova_hover["ui_mode:pressure", "DenDF"]
      p_int <- anova_hover["ui_mode:pressure", "Pr(>F)"]
      cat("**UI mode × Pressure interaction:** ", format_F_apa(F_int, df1_int, df2_int, p_int), "\n\n")
    }
    
    # Estimated marginal means (back-transformed to ms)
    emm_hover <- emmeans(m_hover, ~ ui_mode | pressure, type = "response")
    emm_hover_summary <- summary(emm_hover)
    
    cat("### Estimated Marginal Means\n\n")
    print(kable(emm_hover_summary, digits = 1, 
                caption = "Hover/Dwell Time (ms) by UI Mode and Pressure"))
    cat("\n")
    
    # Pairwise comparisons
    pairs_hover <- pairs(emm_hover, adjust = "holm")
    pairs_hover_df <- as.data.frame(pairs_hover)
    
    if (nrow(pairs_hover_df) > 0) {
      pairs_hover_apa <- pairs_hover_df %>%
        mutate(
          `p-value` = format_p_apa(p.value),
          `t-ratio` = round(t.ratio, 2),
          `Ratio` = round(ratio, 3)
        ) %>%
        select(contrast, `Ratio`, SE, `t-ratio`, `p-value`, df)
      
      cat("### Pairwise Comparisons (Holm-adjusted)\n\n")
      print(kable(pairs_hover_apa, digits = 3, 
                  caption = "UI Mode comparisons within each Pressure condition (ratio scale)"))
      cat("\n")
      
      # APA-formatted text summary
      cat("**APA-formatted summary:** ")
      sig_pairs <- pairs_hover_df %>% filter(p.value < 0.05)
      if (nrow(sig_pairs) > 0) {
        for (i in 1:min(2, nrow(sig_pairs))) {
          press <- sig_pairs$pressure[i]
          ratio <- sig_pairs$ratio[i]
          t_val <- round(sig_pairs$t.ratio[i], 2)
          df_val <- round(sig_pairs$df[i], 1)
          p_val <- sig_pairs$p.value[i]
          pct_change <- round((ratio - 1) * 100, 1)
          cat(sprintf("Under %s pressure, adaptive UI %s hover time by %.1f%% (ratio = %.3f), %s. ", 
                     ifelse(press == "1" || press == 1, "ON", "OFF"),
                     ifelse(ratio < 1, "reduced", "increased"),
                     abs(pct_change),
                     ratio,
                     format_t_apa(t_val, df_val, p_val)))
        }
      } else {
        cat("No significant differences in hover/dwell time between UI modes (all p > 0.05).")
      }
      cat("\n\n")
    }
  }, error = function(e) {
    cat("⚠ Could not fit Hover/Dwell Time model:", conditionMessage(e), "\n\n")
  })
} else {
  cat("⚠ Insufficient data for Hover/Dwell Time statistical tests.\n\n")
}
```
...
<!-- [SUPPLEMENT - ADVANCED/EXPLORATORY] Cognitive modeling: Advanced decision process modeling -->
...
<!-- [SUPPLEMENT - ADVANCED/EXPLORATORY] Movement kinematics modeling: Advanced control theory analysis -->
...
<!-- [CORE] Critical: Final summary of findings -->
...
Participants reported their input device during demographics collection (mouse, trackpad, or other). For hand modality analyses, we excluded trials from participants who used trackpads (n=`r {stats_demog <- compute_input_device_stats(df_raw); stats_demog$trackpad_n}`, `r round(100 * stats_demog$trackpad_n / (stats_demog$mouse_n + stats_demog$trackpad_n), 1)`% of sample), as trackpad vs mouse is a known confound in pointing task performance (MacKenzie & Jusoh, 2001; Karam et al., 2009). Trackpads have different acceleration curves, precision characteristics, and motor control requirements that can significantly affect hand modality performance.
...
**Exclusion impact:** Approximately `r {stats_excl <- compute_input_device_stats(df_raw %>% filter(practice == "false" | practice == FALSE | is.na(practice))); stats_excl$trackpad_hand_trials}` hand trials from trackpad users were excluded (`r round(100 * stats_excl$trackpad_hand_trials / nrow(df_raw %>% filter(practice == "false" | practice == FALSE | is.na(practice))), 1)`% of total trials). All `r stats_excl$trackpad_gaze_trials` gaze trials from trackpad users were retained. Hand modality analyses include N=`r stats_excl$hand_n` participants (mouse users only); gaze modality analyses include N=`r stats_excl$gaze_n` participants (`r stats_excl$mouse_n` mouse + `r stats_excl$trackpad_n` trackpad users).
