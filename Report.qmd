...
# If drag tasks exist, analyze drag distance
if ("drag_distance" %in% names(df_raw)) {
  df_drag <- df_raw %>%
    filter(
      practice == "false" | practice == FALSE | is.na(practice),
      !is.na(drag_distance),
  ...
  if (nrow(df_drag) > 0) {
    ...
    drag_summary <- df_drag %>%
      ...
      summarise(
        N_Trials = n(),
        Mean_Distance = round(mean(drag_distance, na.rm = TRUE), 1),
        SD_Distance = round(sd(drag_distance, na.rm = TRUE), 1),
        Mean_RT = round(mean(rt_ms, na.rm = TRUE), 1),
        .groups = "drop"
      )
    
    drag_summary %>%
      kable(caption = "Drag Distance Metrics by Condition") %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
  }
}
```

```{r plot-drag-distance}
#| fig-cap: "Drag Distance vs. Movement Time. How drag distance relates to performance. Linear regression with 95% CI."
...
if (exists("df_drag") && nrow(df_drag) > 0) {
  ...
  if (nrow(df_drag_rt) > 0) {
    p2 <- ggplot(df_drag_rt, aes(x = drag_distance, y = rt_s, color = ui_mode)) +
      geom_point(alpha = 0.3, size = 1.5) +
      geom_smooth(method = "lm", se = TRUE, alpha = 0.2, linewidth = 1.2) +
      facet_grid(modality ~ pressure,
                 labeller = labeller(
