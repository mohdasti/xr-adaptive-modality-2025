...
if (exists("df_drag") && nrow(df_drag) > 0) {
  ...
  if (nrow(df_plot) > 0) {
    if (has_hexbin) {
      library(hexbin)
      # Calculate appropriate bin counts for hexbin to maintain proper hexagon shape
      # Account for the log scale transformation by using log-space ranges
      x_range <- log10(range(df_plot$drag_distance, na.rm = TRUE))
      y_range <- log10(range(df_plot$rt_s, na.rm = TRUE))
      x_span <- diff(x_range)
      y_span <- diff(y_range)
      # Adjust bins to maintain hexagon shape on log scale
      bins_x <- 35
      bins_y <- round(35 * (y_span / x_span))
      
      p1 <- ggplot(df_plot, aes(x = drag_distance, y = rt_s, color = ui_mode)) +
        geom_hex(bins = bins_x, alpha = 0.7) +
