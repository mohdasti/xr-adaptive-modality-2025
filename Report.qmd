...
# Load diagnostic summary (generated by scripts/diagnose_width_inflation.R)
diagnostic_paths <- c("outputs/width_inflation_diagnostics_summary.csv", "../outputs/width_inflation_diagnostics_summary.csv")
diag_summary <- NULL

for (path in diagnostic_paths) {
  if (file.exists(path)) {
    diag_summary <- read_csv(path, show_col_types = FALSE)
    cat("Loaded diagnostic summary from:", path, "\n\n")
    break
  }
}

if (is.null(diag_summary)) {
  cat("⚠ Diagnostic summary not found. Run scripts/diagnose_width_inflation.R first.\n")
  cat("Running inline diagnostic...\n\n")
  
  # Fallback: inline diagnostic (reconstruct from df_clean)
  ...
  if (nrow(df_hand_adapt_p1) > 0) {
    # Compute candidate triggers (matching diagnostic script)
  ...
  get_value <- function(metric_name) {
    row <- diag_summary %>% filter(metric == metric_name)
    if (nrow(row) > 0) as.numeric(row$value[1]) else NA
  }
  
  n_streak_ge5 <- get_value("n_streak_ge5")
  pct_streak_ge5 <- get_value("pct_streak_ge5")
  median_max_streak <- get_value("median_max_streak")
  mean_max_streak <- get_value("mean_max_streak")
  pct_scaled_hand_adapt_p1 <- get_value("pct_scaled_hand_adapt_p1")
}
...
key_diagnostics <- data.frame(
  Metric = c(
    ...
    "Participants with max trigger streak >= 5 (hysteresis threshold)",
    "Median max consecutive triggered trials",
    "Mean max consecutive triggered trials"
  ),
  Value = c(
    ifelse(is.null(diag_summary), "0.00%", sprintf("%.2f%%", pct_scaled_hand_adapt_p1)),
    ifelse(is.na(n_streak_ge5), "N/A", sprintf("%.1f%%", pct_streak_ge5)),
    ifelse(is.na(median_max_streak), "N/A", sprintf("%.1f", median_max_streak)),
    ifelse(is.na(mean_max_streak), "N/A", sprintf("%.2f", mean_max_streak))
  )
)

print(kable(key_diagnostics, caption = "Width Inflation Diagnostic Summary (Hand/Adaptive/Pressure=1)"))
cat("\n")

# Conclusion
cat("### Root Cause Analysis\n\n")

if (!is.na(n_streak_ge5) && !is.na(pct_scaled_hand_adapt_p1)) {
  if (pct_scaled_hand_adapt_p1 == 0) {
    if (n_streak_ge5 < 5) {
      cat("**Evidence favors strict hysteresis threshold:** Width scaling did not activate (0% of trials) even though candidate triggers (RT > p75 OR error bursts) occurred frequently. However, only ", 
          sprintf("%.1f%%", pct_streak_ge5), 
          " of participants reached the hysteresis threshold (5 consecutive triggered trials), with median/mean streaks of ", 
          sprintf("%.1f / %.2f", median_max_streak, mean_max_streak), 
          " consecutive trials. This suggests the hysteresis gate (default: 5 consecutive triggers required) was too strict for the task duration and participant performance patterns.\n\n", sep = "")
    } else {
      cat("**Evidence favors integration/policy issue:** Width scaling did not activate (0% of trials) despite ", 
          sprintf("%.1f%%", pct_streak_ge5), 
          " of participants reaching the hysteresis threshold (>=5 consecutive triggered trials). This suggests a pressure gate issue, policy integration problem, or configuration error rather than threshold strictness.\n\n", sep = "")
    }
  }
} else {
  cat("**Conclusion:** Diagnostic data unavailable. See scripts/diagnose_width_inflation.R for full analysis.\n\n")
}

cat("**Note:** The diagnostic script reconstructs candidate triggers using policy-like logic: (RT > participant p75) OR (error burst >= 2). The policy requires 5 consecutive triggered trials (hysteresis_trials=5) before activating width inflation.\n\n")
```
