...
```{r results-snapshot}
# Gaze-only: adaptive - static contrasts (optionally by pressure)
gaze_snapshot <- df_pid_cond %>%
  filter(modality == "gaze") %>%
  group_by(modality, ui_mode, pressure) %>%
  summarise(
...
# Hand-only: pressure effect contrasts (ui_mode excluded by design)
hand_snapshot <- df_pid_cond %>%
...
gaze_snapshot %>%
  kable(caption = "Gaze contrasts: adaptive - static (by pressure)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)

cat("\n### Hand: Pressure Effect (UI Mode Not Exercised)\n\n")
hand_snapshot %>%
  kable(caption = "Hand contrasts: pressure ON - pressure OFF") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
...
if (exists("model_tp") && !is.null(model_tp)) {
  ...
  if (exists("model_tp_gaze") && !is.null(model_tp_gaze) && exists("emm_tp_gaze") && !is.null(emm_tp_gaze)) {
    cat("#### Gaze-Only Follow-up: UI Mode × Pressure (Primary Adaptive Test)\n\n")
...
cat("### Diagnostic: Why Did Hand Width Scaling Not Activate?\n\n")

# 1. Check for trigger/threshold/policy columns in df_raw
trigger_cols <- grep("trigger|threshold|hyster|policy|adapt|scale", names(df_raw), ignore.case = TRUE, value = TRUE)
cat("**Trigger-related columns in df_raw:**\n")
if (length(trigger_cols) > 0) {
  cat(paste("-", trigger_cols, collapse = "\n"), "\n\n")
  
  # Summarize trigger occurrences by modality/ui_mode/pressure (focus on hand/adaptive/pressure ON)
  ...
  if (nrow(df_hand_adapt_p1) > 0) {
    cat("**Trigger summary for HAND/Adaptive/Pressure ON:**\n")
    for (col in trigger_cols) {
      if (col %in% names(df_hand_adapt_p1)) {
        cat("\n-", col, ":\n")
        # Check if column is numeric
        is_numeric_col <- is.numeric(df_hand_adapt_p1[[col]]) || 
                          all(grepl("^[0-9.]+$", as.character(df_hand_adapt_p1[[col]])), na.rm = TRUE)
        
        if (is_numeric_col) {
          # Convert to numeric if needed
          df_hand_adapt_p1[[col]] <- as.numeric(df_hand_adapt_p1[[col]])
          summary_data <- df_hand_adapt_p1 %>%
            group_by(pid) %>%
            summarise(
              mean_val = mean(.data[[col]], na.rm = TRUE),
              median_val = median(.data[[col]], na.rm = TRUE),
              max_val = max(.data[[col]], na.rm = TRUE),
              pct_nonzero = 100 * mean(.data[[col]] != 0, na.rm = TRUE),
              .groups = "drop"
            )
          print(knitr::kable(summary_data %>% head(10), digits = 2))
        } else {
          # For character columns, show unique values and counts
          summary_data <- df_hand_adapt_p1 %>%
            group_by(pid) %>%
            summarise(
              unique_vals = paste(unique(.data[[col]][!is.na(.data[[col]])]), collapse = ", "),
              n_non_na = sum(!is.na(.data[[col]])),
              .groups = "drop"
            ) %>%
            filter(n_non_na > 0)
          if (nrow(summary_data) > 0) {
            print(knitr::kable(summary_data %>% head(10)))
          } else {
    ...
    # Check if scaling followed triggers
    if ("width_scale_factor" %in% names(df_hand_adapt_p1)) {
      scaling_by_trigger <- df_hand_adapt_p1 %>%
        group_by(across(any_of(trigger_cols))) %>%
        summarise(
          n_trials = n(),
          mean_width_scale = mean(width_scale_factor, na.rm = TRUE),
          pct_scaled = 100 * mean(width_scale_factor != 1, na.rm = TRUE),
          .groups = "drop"
        )
      cat("\n**Scaling following triggers:**\n")
      print(knitr::kable(scaling_by_trigger %>% head(10), digits = 2))
    }
  }
} else {
  cat("No trigger/threshold/policy columns found in df_raw.\n\n")
  
  # 2. Reconstruct candidate triggers from RT and error streaks
  cat("**Reconstructing candidate triggers from RT and error patterns...**\n\n")
  ...
  if (nrow(df_hand_adapt_p1) > 0) {
    # Compute per-participant p75 RT
    p75_rt_by_pid <- df_hand_adapt_p1 %>%
      group_by(pid) %>%
      summarise(p75_rt = quantile(rt_ms, 0.75, na.rm = TRUE), .groups = "drop")
    
    # Merge p75 RT back
    df_hand_adapt_p1 <- df_hand_adapt_p1 %>%
      left_join(p75_rt_by_pid, by = "pid") %>%
      mutate(
        is_slow = rt_ms > p75_rt
      )
    
    # Compute rolling slow streak lengths and rolling consecutive error lengths
    df_candidate_triggers <- df_hand_adapt_p1 %>%
      group_by(pid) %>%
      arrange(trial_number) %>%
      mutate(
        slow_streak = sequence(rle(is_slow)$lengths) * is_slow,
        error_streak = sequence(rle(error)$lengths) * error
      ) %>%
      ungroup() %>%
      mutate(
        candidate_trigger_n2 = (slow_streak >= 2) | (error_streak >= 2),
        candidate_trigger_n3 = (slow_streak >= 3) | (error_streak >= 2)
      )
    
    # Count candidate triggers per participant
    trigger_summary <- df_candidate_triggers %>%
      group_by(pid) %>%
      summarise(
        n_trials = n(),
        n_candidate_n2 = sum(candidate_trigger_n2, na.rm = TRUE),
        n_candidate_n3 = sum(candidate_trigger_n3, na.rm = TRUE),
        n_observed_scaling = sum(width_scale_factor != 1, na.rm = TRUE),
        pct_candidate_n2 = 100 * mean(candidate_trigger_n2, na.rm = TRUE),
        pct_candidate_n3 = 100 * mean(candidate_trigger_n3, na.rm = TRUE),
        .groups = "drop"
      )
    
    cat("**Candidate triggers per participant (N=2: slow_streak>=2 OR error_streak>=2; N=3: slow_streak>=3 OR error_streak>=2):**\n")
    print(knitr::kable(trigger_summary %>% head(20), digits = 2))
    
    # Overall summary
    overall_summary <- df_candidate_triggers %>%
      summarise(
        total_trials = n(),
        total_candidate_n2 = sum(candidate_trigger_n2, na.rm = TRUE),
        total_candidate_n3 = sum(candidate_trigger_n3, na.rm = TRUE),
        total_observed_scaling = sum(width_scale_factor != 1, na.rm = TRUE),
        pct_candidate_n2 = 100 * mean(candidate_trigger_n2, na.rm = TRUE),
        pct_candidate_n3 = 100 * mean(candidate_trigger_n3, na.rm = TRUE),
        pct_scaled = 100 * mean(width_scale_factor != 1, na.rm = TRUE)
      )
    
    cat("\n**Overall summary (HAND/Adaptive/Pressure ON):**\n")
    print(knitr::kable(overall_summary, digits = 2))
    
    # Conclusion
    cat("\n**Conclusion:**\n")
    if (overall_summary$total_candidate_n2 > 0 || overall_summary$total_candidate_n3 > 0) {
      if (overall_summary$total_observed_scaling == 0) {
        cat("**Evidence favors bug/misconfig:** ", 
            overall_summary$total_candidate_n2, " candidate triggers (N=2) and ",
            overall_summary$total_candidate_n3, " candidate triggers (N=3) exist, but NO scaling was observed (width_scale_factor always 1). ",
            "This suggests the adaptation policy logic may not be executing correctly, or there is a configuration issue.\n", sep = "")
      } else {
        cat("**Evidence favors strict thresholds:** Some candidate triggers exist (",
            overall_summary$total_candidate_n2, " with N=2, ", overall_summary$total_candidate_n3, " with N=3), ",
            "but only ", overall_summary$total_observed_scaling, " trials showed scaling. ",
            "This suggests hysteresis gates (N consecutive triggers required) or strict threshold settings.\n", sep = "")
      }
    } else {
      cat("**Insufficient evidence:** No candidate triggers detected. This could indicate either:\n")
      cat("(1) Participants performed well (RTs did not exceed p75, errors did not form streaks)\n")
      cat("(2) Thresholds are set very strictly\n")
      cat("(3) RT/error data not available for trigger reconstruction\n")
    }
  } else {
    cat("⚠ No HAND/Adaptive/Pressure ON trials found for diagnostic.\n")
  }
}
