...
cat("**Current Status:** This report is based on an interim sample of N=`r n_distinct(df$pid)` participants. All power assessments below assume the final target of N=48 unless otherwise noted.\n\n")
...
**Interim Status:** All inferential models in this document are fit on N=`r n_distinct(df$pid)` (interim sample), not N=48. Any statements about N=48 refer to the *planned* final sample, not the data used here.
:::
...
qc_participants %>%
  kable(caption = "Participant counts (interim). Note: 'df' refers to correct trials, RT ∈ [150, 6000] ms, non-practice.") %>%
...
**Current dataset: N=`r n_distinct(df$pid)` participants (interim sample).** All inferential results in this document are preliminary and will be re-estimated at N≈48. Sections 16 (LBA) and 17 (Control Theory) document planned analyses that have not yet been implemented at the current interim N.
:::
...
**Note on Participant Exclusions:** Seven participants (P002, P003, P007, P008, P015, P039, P040) were excluded from the *planned* 2×2×2 factorial dataset due to a data logging error that incorrectly recorded pressure conditions. The bug was fixed on December 8, 2025 (commit `04758db`), and seven replacement participants (P049-P055) will be collected to reach the planned final sample of N=48. **All analyses in this interim report use N=`r n_distinct(df$pid)` participants with complete data across all conditions.**

**All statistical models in this document are fit on N=`r n_distinct(df$pid)` (interim sample), not N=48.** See the Data Quality Notes section and `EXCLUSION_CRITERIA.md` for details.
...
if (exists("model_tp") && !is.null(model_tp)) {
  ...
  cat("\n**Interim Analysis Note:** At N=", diag_tp$n_participants, ", the model is underpowered for detecting 3-way interactions. Any non-significant interaction effects should be treated as exploratory and will be revisited at N=48.\n\n")
...
cat("\n**Error Rate Summary:** Overall error rate was ", round(100 * overall_error_rate, 1), "%. Errors were concentrated in gaze conditions (", round(100 * gaze_error_rate, 1), "%), while hand remained near ", round(100 * hand_error_rate, 1), "%. At this interim N and low error count, we report these differences descriptively.\n\n")
...
**Data Availability Note:** Submovement metrics are available for a subset of the interim sample (see counts below). All results in this section are descriptive and should be treated as preliminary engineering diagnostics, not inferential findings. We distinguish between:
...
if ("err_type" %in% names(df_raw)) {
  ...
  if (nrow(df_error_types) > 0) {
    ...
    error_type_summary %>%
      kable(caption = paste0("Error Type Distribution by Condition (", get_n_string(df_error_types), " participants)")) %>%
      kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE)
    
    # Interpretive summary
    gaze_miss_pct <- df_error_types %>%
      filter(modality == "gaze", err_type == "miss") %>%
      nrow() / df_error_types %>% filter(modality == "gaze") %>% nrow() * 100
    hand_miss_pct <- df_error_types %>%
      filter(modality == "hand", err_type == "miss") %>%
      nrow() / df_error_types %>% filter(modality == "hand") %>% nrow() * 100
    
    cat("\n**Error Type Summary:** Gaze produced more misses (", round(gaze_miss_pct, 1), "%) than hand (", round(hand_miss_pct, 1), "%), consistent with its lower throughput. Adaptive UI did not yet show a clear reduction in any specific error type at N=", n_distinct(df_error_types$pid), ".\n\n")
  }
}
```
  ...
  if (nrow(df_gate) > 0) {
    ...
    cat("\n**Alignment Gate Interpretation:** False triggers were ", ifelse(overall_false_triggers < 0.5, "rare", ifelse(overall_false_triggers < 2, "moderate", "frequent")), 
        ...
        ifelse(abs(static_false_triggers - adaptive_false_triggers) < 0.1, 
               "Adaptive UI did not show a meaningful change in false trigger rate compared to Static at this interim N.", 
               paste0("Adaptive UI showed a ", ifelse(adaptive_false_triggers < static_false_triggers, "reduction", "increase"), 
                      ...
                      "), but CIs are wide at N=", n_distinct(df_gate$pid), " and this will be revisited at N=48.")), "\n\n")
  } else {
...
if (exists("df_gate") && nrow(df_gate) > 0) {
  ...
  if (sum(!is.na(df_gate$alignment_gate_false_triggers)) > 0) {
    ...
    # Ensure all modality/pressure combinations are represented (even if empty)
    # This ensures all facets are shown in the plot
    all_combinations <- expand_grid(
      modality = unique(df_gate$modality),
      pressure = unique(df_gate$pressure),
      ui_mode = c("static", "adaptive")
    )
    
    # Left join to ensure all combinations are present (missing ones will have NA values)
    # This ensures facet_grid shows all facets, even if some are empty
    df_gate_plot_complete <- all_combinations %>%
      left_join(df_gate_plot, by = c("modality", "pressure", "ui_mode")) %>%
      mutate(
        modality = factor(modality, levels = levels(df_gate$modality)),
        pressure = factor(pressure, levels = levels(df_gate$pressure)),
        ui_mode = factor(ui_mode, levels = c("static", "adaptive"))
      )
    
    # Use complete data for plotting (will show empty facets for missing combinations)
    df_gate_plot <- df_gate_plot_complete
...
if (exists("df_gate") && nrow(df_gate) > 0) {
  ...
  if (has_recovery_data) {
    ...
    if (nrow(df_gate_recovery) > 0) {
      ...
      # Create complete grid of all combinations to ensure all facets are shown
      all_combinations <- expand_grid(
        modality = unique(df_gate$modality),
        pressure = unique(df_gate$pressure),
        ui_mode = c("static", "adaptive")
      ...
      # Left join to ensure all combinations are present (missing ones will have NA values)
      # This ensures facet_grid shows all facets, even if some are empty
      df_gate_recovery_plot_complete <- all_combinations %>%
        left_join(df_gate_recovery_plot, by = c("modality", "pressure", "ui_mode")) %>%
        mutate(
          modality = factor(modality, levels = levels(df_gate$modality)),
          pressure = factor(pressure, levels = levels(df_gate$pressure)),
          ui_mode = factor(ui_mode, levels = c("static", "adaptive"))
      ...
      # Use complete data for plotting (will show empty facets for missing combinations)
      df_gate_recovery_plot <- df_gate_recovery_plot_complete
...
if (exists("df_gate") && nrow(df_gate) > 0) {
  ...
  if (has_mean_recovery_data) {
    ...
    if (nrow(df_gate_mean_recovery) > 0) {
      ...
      # Left join to ensure all combinations are present (missing ones will have NA values)
      # This ensures facet_grid shows all facets, even if some are empty
      df_gate_mean_recovery_plot_complete <- all_combinations %>%
        left_join(df_gate_mean_recovery_plot, by = c("modality", "pressure", "ui_mode")) %>%
        mutate(
          modality = factor(modality, levels = levels(df_gate$modality)),
          pressure = factor(pressure, levels = levels(df_gate$pressure)),
          ui_mode = factor(ui_mode, levels = c("static", "adaptive"))
      ...
      # Use complete data for plotting (will show empty facets for missing combinations)
      df_gate_mean_recovery_plot <- df_gate_mean_recovery_plot_complete
  ...
  if (nrow(df_hover) > 0 && n_distinct(df_hover$pid) > 0) {
    ...
    if (length(static_mean) > 0 && length(adaptive_mean) > 0) {
      ...
      cat("\n**Hover Time Summary:** Adaptive UI ", ifelse(diff_ms < 0, "reduced", "increased"), 
          ...
          " ms, Adaptive: ", round(adaptive_mean, 0), " ms). However, CIs are wide at this interim N=", 
          n_distinct(df_hover$pid), " and this will be revisited at N=48.\n\n")
    } else {
      cat("\n**Hover Time Summary:** At this interim N=", n_distinct(df_hover$pid), 
          ", interpretation is limited due to small sample size. This analysis will be revisited at N=48.\n\n")
    }
  } else {
...
**Status:** ⚠️ **PLANNED ANALYSIS - No LBA models fit yet at current interim N (N=`r n_distinct(df$pid)`).**
...
cat("This section will analyze verification-time RTs using hierarchical Linear Ballistic Accumulator models.\n\n")
cat("**Current Status:**\n")
cat("- Basic verification_time_ms analysis: ✅ DONE (Section 10)\n")
cat("- LBA model fitting: ❌ PENDING\n")
cat("- Hierarchical parameter estimation: ❌ PENDING\n\n")
cat("**Implementation Plan:**\n")
cat("- LBA requires RT data from the verification phase (time from target entry to selection)\n")
cat("- Model fitting can be done using `RWiener` or `rtdists` packages\n")
cat("- Key parameters to estimate: drift rate (v), threshold (b), starting point (A), non-decision time (t0)\n")
cat("- Hypothesis: Adaptive conditions should show lower threshold (b), indicating less caution needed\n")
cat("- Hierarchical modeling will account for participant-level variation\n\n")
...
cat("- Requires sufficient trial counts per condition for stable parameter estimation\n")
cat("- Will be implemented once N reaches target sample size\n\n")
cat("**Power Considerations:**\n")
cat("- N=48 is sufficient for medium main effects (dz≈0.41, power≈0.80)\n")
cat("- LBA parameters require careful convergence diagnostics\n")
cat("- See `POWER_ANALYSIS_EXPERT_RESPONSE.md` for detailed recommendations\n")
```
...
**Status:** ⚠️ **PLANNED ANALYSIS - No trajectory-based models fit yet at current interim N (N=`r n_distinct(df$pid)`).**
...
summary_table %>%
  kable(caption = paste0("Summary of Key Metrics by Condition (Interim N=", n_distinct(df$pid), ")")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = FALSE) %>%
...
- Seven replacement participants (P049-P055) will be collected to maintain planned N=48
- Affected participants' data retained for exploratory analyses

**Current Sample (Interim):** N=`r n_distinct(df$pid)` participants with complete data across all experimental conditions.
...
**Note:** All summary statistics above are based on the current interim sample (N=`r n_distinct(df$pid)`). Effect sizes and p-values may change as more data are collected.
