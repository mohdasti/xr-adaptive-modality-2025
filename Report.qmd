  ...
  if (nrow(df_filtered) > 0) {
    ...
    if (length(id_quantiles) >= 2) {
      ...
      if (n_bins >= 2) {
        ...
        # Note: cut() creates length(breaks) - 1 intervals, so labels must match
        if (n_bins == 4 && length(id_quantiles) >= 5) {
          breaks <- id_quantiles
          labels <- c("Q1 (Low)", "Q2", "Q3", "Q4 (High)")  # 4 labels for 4 intervals
        } else if (n_bins == 3 && length(id_quantiles) >= 4) {
          ...
          # 2 bins: need 3 break points for 2 intervals
        ...
        # Only proceed if we have valid breaks
